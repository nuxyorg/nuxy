...
describe('ollama backend', () => {
  ...
  describe('eval handler', () => {
    ...
    it('returns Ask Ollama item with execute action', async () => {
      ...
      expect(core.i18n.t).toHaveBeenCalledWith('provider.askOllama')
      expect(core.i18n.t).toHaveBeenCalledWith('provider.subtitle', {
        text: 'explain quantum computing',
      })
    })
  })
  ...
  describe('chat handler', () => {
    ...
    it('prepends the configured system prompt as the first message', async () => {
      vi.restoreAllMocks()
      vi.spyOn(global, 'fetch').mockReturnValue(
        makeFetchOk({ message: { role: 'assistant', content: 'ok' } }) as ReturnType<typeof fetch>
      )
    ...
    it('includes temperature in options when configured', async () => {
      vi.restoreAllMocks()
      vi.spyOn(global, 'fetch').mockReturnValue(
        makeFetchOk({ message: { role: 'assistant', content: 'ok' } }) as ReturnType<typeof fetch>
      )
  ...
  describe('configure handler', () => {
    ...
    it('writes thinkingColor, systemPrompt, and temperature when provided', async () => {
      const { core, handlers } = createCore()
      await register(core)
      await (handlers['configure'] as (p: unknown) => Promise<void>)({
        thinkingColor: 'rainbow',
        systemPrompt: 'Be terse.',
        temperature: 0.9,
      })
      expect(core.settings.write).toHaveBeenCalledWith('thinkingColor', 'rainbow')
      expect(core.settings.write).toHaveBeenCalledWith('systemPrompt', 'Be terse.')
      expect(core.settings.write).toHaveBeenCalledWith('temperature', 0.9)
    })
  })
