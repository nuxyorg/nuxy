...
export async function register(core: CoreContext): Promise<void> {
  ...
  core.ipc.handle('openWithQuery', async (payload: unknown): Promise<OpenWithQueryResult> => {
    const { text } = payload as OpenWithQueryPayload
    return { toolId: EXT_ID, query: text }
  })
  ...
  core.ipc.handle('eval', async (payload: unknown): Promise<EvalResult> => {
    ...
    return {
      items: [
        {
          id: EXT_ID,
          title: core.i18n.t('provider.askOllama'),
          subtitle: core.i18n.t('provider.subtitle', { text }),
          execute: {
  ...
  core.ipc.handle('history:save', async (payload: unknown): Promise<void> => {
    const { messages } = payload as HistorySavePayload
    await core.storage.write<ChatMessage[]>('history.json', messages)
  })
  ...
  core.ipc.handle('history:clear', async (): Promise<void> => {
    await core.storage.write<ChatMessage[]>('history.json', [])
  })
}
