...
function createCore(): { core: CoreContext; handlers: Record<string, (payload?: unknown) => Promise<unknown>> } {
  const handlers: Record<string, (payload?: unknown) => Promise<unknown>> = {}
  return {
    core: {
      registry: { registerProvider: vi.fn(), registerTool: vi.fn(), registerOrchestrator: vi.fn(), registerTheme: vi.fn(), registerIconPack: vi.fn() },
  ...
  describe('eval handler', () => {
    ...
    describe('valid arithmetic', () => {
      it('returns one result item for simple addition', async () => {
        const res = await handlers.eval({ text: '2 + 2' }) as { items: Array<{ id: string; title: string; subtitle: string; value: number }> }
      ...
      it('trims leading and trailing whitespace before evaluating', async () => {
        const res = await handlers.eval({ text: '  2 + 2  ' }) as { items: Array<{ value: number }> }
        expect(res.items).toHaveLength(1)
