...
function createCore({ storageData = null, clipText = '', clipImage = null } = {}) {
  const handlers = {}
  const core = {
    ...
    logger: { info: vi.fn(), error: vi.fn(), silly: vi.fn() },
  }
...
describe('clipboard backend', () => {
  ...
  describe('getHistory', () => {
    it('returns empty array before init resolves', async () => {
      ...
      register({ registry: { registerTool: vi.fn() }, ipc: { handle: (ch, fn) => { handlers[ch] = fn } }, storage: { read: vi.fn().mockResolvedValue(null) }, clipboard: { readText: vi.fn().mockResolvedValue(''), readImage: vi.fn().mockResolvedValue(null) }, fs: { fileExists: vi.fn() }, logger: { info: vi.fn(), error: vi.fn(), silly: vi.fn() } })
  ...
  describe('pinItem', () => {
    it('marks an item as pinned', async () => {
      ...
      const result = await handlers.pinItem('a')
    ...
    it('moves pinned items to the front', async () => {
      ...
      const result = await handlers.pinItem('b')
    ...
    it('does nothing if item not found', async () => {
      ...
      const result = await handlers.pinItem('nonexistent')
      expect(result).toHaveLength(1)
      expect(result[0].id).toBe('a')
    })
  ...
  describe('unpinItem', () => {
    it('marks a pinned item as unpinned', async () => {
      ...
      const result = await handlers.unpinItem('a')
  ...
  describe('deleteItem', () => {
    it('removes the item from history', async () => {
      ...
      const result = await handlers.deleteItem('a')
    ...
    it('does nothing if id not found', async () => {
      ...
      const result = await handlers.deleteItem('ghost')
      expect(result).toHaveLength(1)
    })
  ...
  describe('copyItem', () => {
    ...
    it('moves the copied item to the top of history', async () => {
      ...
      const result = await handlers.copyItem('b')
      expect(result[0].id).toBe('b')
    })
    ...
    it('returns unchanged history if id not found', async () => {
      ...
      const result = await handlers.copyItem('ghost')
      expect(result).toHaveLength(1)
    })
  ...
  describe('copyFile', () => {
    ...
    it('returns updated history with the copied item moved to top', async () => {
      ...
      const result = await handlers.copyFile('b')
      expect(result[0].id).toBe('b')
    })
    ...
    it('returns current history unchanged if item id not found', async () => {
      ...
      const result = await handlers.copyFile('ghost')
      expect(result).toHaveLength(1)
      expect(result[0].id).toBe('a')
  ...
  describe('sortHistory', () => {
    it('pinned items appear before unpinned after a pin operation', async () => {
      ...
      const result = await handlers.pinItem('c')
    ...
    it('mixed pin/unpin operations maintain correct ordering throughout', async () => {
      ...
      const afterUnpin = await handlers.unpinItem('b')
  ...
  describe('e2e: clipboard lifecycle', () => {
    it('pin item, clear history, pinned item survives', async () => {
      ...
      await handlers.pinItem('a')
      const afterClear = await handlers.clearHistory()
    ...
    it('pin, unpin, pin again — ordering stays consistent', async () => {
      ...
      await handlers.pinItem('a')
      expect((await handlers.getHistory())[0].pinned).toBe(true)

      await handlers.unpinItem('a')
      expect((await handlers.getHistory())[0].pinned).toBe(false)
