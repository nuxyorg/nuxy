...
describe('clipboard backend', () => {
  ...
  describe('pinItem', () => {
    ...
    it('persists the pin to storage', async () => {
      ...
      register(core)
      await flush()
      ;(core.storage.write as ReturnType<typeof vi.fn>).mockClear()
  ...
  describe('checkFile', () => {
    it('returns true when the file exists on disk', async () => {
      ...
      ;(core.fs.fileExists as ReturnType<typeof vi.fn>).mockResolvedValue(true)
      register(core)
      await flush()
    ...
    it('returns false when the file does not exist', async () => {
      const { core, handlers } = createCore()
      ;(core.fs.fileExists as ReturnType<typeof vi.fn>).mockResolvedValue(false)
      register(core)
      await flush()
  ...
  describe('addHistoryItem (deduplication and cap)', () => {
    it('adding same text twice keeps only one entry, most recent at top', async () => {
      ...
      ;(core.clipboard.readText as ReturnType<typeof vi.fn>).mockResolvedValue('duplicate text')
      ;(core.clipboard.readImage as ReturnType<typeof vi.fn>).mockResolvedValue(null)
    ...
    it('adding same image twice keeps only one entry', async () => {
      ...
      ;(core.clipboard.readImage as ReturnType<typeof vi.fn>).mockResolvedValue('img-data-abc')
      ;(core.clipboard.readText as ReturnType<typeof vi.fn>).mockResolvedValue('')
      await vi.advanceTimersByTimeAsync(1000)
      await flush()
      ...
      ;(core.clipboard.readImage as ReturnType<typeof vi.fn>).mockResolvedValue('img-data-xyz') // different image
      await vi.advanceTimersByTimeAsync(1000)
      await flush()
      ;(core.clipboard.readImage as ReturnType<typeof vi.fn>).mockResolvedValue('img-data-abc') // same as first
      await vi.advanceTimersByTimeAsync(1000)
      await flush()
    ...
    it('the 100-item cap evicts oldest unpinned items but preserves pinned ones', async () => {
      ...
      ;(core.clipboard.readText as ReturnType<typeof vi.fn>).mockResolvedValue('brand-new-item')
      ;(core.clipboard.readImage as ReturnType<typeof vi.fn>).mockResolvedValue(null)
      await vi.advanceTimersByTimeAsync(1000)
      await flush()
  ...
  describe('clipboard polling', () => {
    it('adds a new text entry to history when clipboard changes after one tick', async () => {
      ...
      ;(core.clipboard.readText as ReturnType<typeof vi.fn>).mockResolvedValue('polled text')
      ;(core.clipboard.readImage as ReturnType<typeof vi.fn>).mockResolvedValue(null)
    ...
    it('does not add an entry when clipboard text is unchanged between ticks', async () => {
      ...
      ;(core.clipboard.readText as ReturnType<typeof vi.fn>).mockResolvedValue('stable text')
      ;(core.clipboard.readImage as ReturnType<typeof vi.fn>).mockResolvedValue(null)
      await vi.advanceTimersByTimeAsync(1000)
      await flush()
