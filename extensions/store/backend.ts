import type { CoreContext } from '@nuxyorg/extension-sdk'
import type {
  ActionResult,
  ExtensionListItem,
  InstallExtensionPayload,
  RegistryIndex,
  StoreExtension,
  UninstallExtensionPayload,
} from './types.ts'

const DEFAULT_REGISTRY_URL =
  'https://raw.githubusercontent.com/atagulalan/nuxy-assets/main/registry.json'

/** Minimal shape of a kernel `listInstalledExtensions` entry that this extension reads. */
interface LoadedExtensionSummary {
  id: string
  manifest: {
    name: string
    version: string
    type: StoreExtension['type']
    permissions?: string[]
    icon?: string
    bootstrap?: boolean
  }
}

function isSystemExtension(id: string, bootstrap?: boolean): boolean {
  return id === 'com.nuxy.shell' || id === 'com.nuxy.settings' || !!bootstrap
}

export function register(core: CoreContext): void {
  core.registry.registerTool({ name: 'store', displayName: 'Store' })

  // Fetches remote registry JSON and installed extensions, then merges them.
  core.ipc.handle('getExtensions', async (): Promise<ExtensionListItem[]> => {
    try {
      const registryUrl = (await core.settings.read<string>('registryUrl')) ?? DEFAULT_REGISTRY_URL

      // 1. Fetch remote registry.
      let remoteExtensions: StoreExtension[] = []
      try {
        const response = await fetch(registryUrl)
        if (response.ok) {
          const index = (await response.json()) as RegistryIndex
          remoteExtensions = index.extensions || []
        }
      } catch (err) {
        core.logger.error('Failed to fetch store registry index:', err)
      }

      // 2. Fetch installed extensions from the kernel. This depends on the
      // kernel-side `listInstalledExtensions` channel — verified present in
      // src/electron/ipc/kernel-invokable.ts at port time.
      let installedExtensions: LoadedExtensionSummary[] = []
      const kernelRes = await core.extensions.invoke('kernel', 'listInstalledExtensions')
      if (kernelRes.success && Array.isArray(kernelRes.data)) {
        installedExtensions = kernelRes.data as LoadedExtensionSummary[]
      }

      // 3. Map installed extensions for lookup.
      const installedMap = new Map<string, LoadedExtensionSummary>()
      for (const ext of installedExtensions) {
        installedMap.set(ext.id, ext)
      }

      const mergedList: ExtensionListItem[] = []

      // 4. Process all extensions from the remote catalog.
      for (const remote of remoteExtensions) {
        const installed = installedMap.get(remote.id)
        const installedVersion = installed?.manifest.version

        mergedList.push({
          ...remote,
          installed: !!installed,
          installedVersion,
          canUpdate: !!installed && installedVersion !== remote.version,
          isSystem: isSystemExtension(remote.id, installed?.manifest.bootstrap),
        })
        installedMap.delete(remote.id)
      }

      // 5. Append any locally installed extensions that aren't in the remote catalog.
      for (const [, local] of installedMap) {
        mergedList.push({
          id: local.id,
          name: local.manifest.name,
          description: 'Locally installed extension.',
          version: local.manifest.version,
          type: local.manifest.type,
          author: 'Local',
          downloadUrl: '',
          permissions: local.manifest.permissions,
          icon: local.manifest.icon,
          installed: true,
          installedVersion: local.manifest.version,
          canUpdate: false,
          isSystem: isSystemExtension(local.id, local.manifest.bootstrap),
        })
      }

      return mergedList
    } catch (e) {
      core.logger.error('Error merging store extensions:', e)
      return []
    }
  })

  // Proxies install requests to the kernel.
  //
  // NOTE: this depends on `core.extensions.invoke('kernel', 'installExtension', ...)`,
  // which is implemented in src/electron/ipc/kernel-invokable.ts (downloads the
  // .nuxyext file and triggers a rescan). Confirmed present at port time.
  core.ipc.handle('installExtension', async (payload: unknown): Promise<ActionResult> => {
    const { extId, downloadUrl } = payload as InstallExtensionPayload
    const result = await core.extensions.invoke('kernel', 'installExtension', {
      extId,
      downloadUrl,
    })
    return {
      success: result.success,
      error: result.error,
    }
  })

  // Proxies uninstall requests to the kernel.
  //
  // NOTE: this depends on `core.extensions.invoke('kernel', 'uninstallExtension', ...)`,
  // which is implemented in src/electron/ipc/kernel-invokable.ts (rejects
  // com.nuxy.shell / com.nuxy.settings / bootstrap extensions). Confirmed
  // present at port time.
  core.ipc.handle('uninstallExtension', async (payload: unknown): Promise<ActionResult> => {
    const { extId } = payload as UninstallExtensionPayload
    const result = await core.extensions.invoke('kernel', 'uninstallExtension', { extId })
    return {
      success: result.success,
      error: result.error,
    }
  })
}
