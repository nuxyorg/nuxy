...
interface NavSection { id: string; label: string; itemCount: number }
...
export default function VideoDownloader({ query }: Props) {
  ...
  const { filteredFormats, combinedList, stateRef } = useVideoMeta({
    metadata, activeTab, jobs, history, url, lastUrl, loading,
    selectedIndex, downloadSelectedIndex, previousFormatTab,
  })

  // ── Nav ──────────────────────────────────────────────────────────────────────
  const navSections = React.useMemo(() => TABS.map((t) => ({ id: t.id, label: t.label, itemCount: 1 })), [])
  const rightPanelRef = React.useRef<HTMLDivElement | null>(null)
  const nav = _useTwoPanelNav({
    sections: navSections,
    initialFocusArea: 'right',
    onSectionChange: (id: string) => { setActiveTab(id as TabId); setSelectedIndex(-1) },
  ...
  const actions = useVideoActions({
    url: stateRef.current.url, metadata, filteredFormats, activeTab,
    setMetadata, setLoading, setError, setLastUrl, setActiveTab, setSelectedIndex,
    setJobs, setPreviousFormatTab, setDownloadSelectedIndex, goToSection: nav.goToSection,
  })

  // ── Keyboard + palette ────────────────────────────────────────────────────────
  useVideoKeyboard({
    stateRef, setSelectedIndex, setDownloadSelectedIndex, setActiveTab,
    setJobs, setPreviousFormatTab, metadata, actions,
    goToSection: nav.goToSection, setFocusArea: nav.setFocusArea,
  })

  // ── Shell sync ────────────────────────────────────────────────────────────────
  useVideoSync({
    url, lastUrl, metadata, loading, jobs, focusArea, activeTab, activeSectionId,
    downloadSelectedIndex, combinedList, filteredFormatsLength: filteredFormats.length,
    loadHistory, setMetadata, setError, setActiveTab, setSelectedIndex,
    setDownloadSelectedIndex, setPreviousFormatTab, ShortcutSep,
  })

  // ── Views ─────────────────────────────────────────────────────────────────────
  if (activeTab === 'downloads')
    return <VideoDownloadsList combinedList={combinedList} downloadSelectedIndex={downloadSelectedIndex} />
  ...
  // ── Main layout ───────────────────────────────────────────────────────────────
  const left = TabBar
    ? <TabBar tabs={TABS} active={activeTab} orientation="vertical" onChange={(id: string) => nav.goToSection(id)} />
    : null

  const right = (ScrollArea as unknown) && (Box as unknown) ? (
    <ScrollArea ref={rightPanelRef} style={{ flex: 1 }}>
      <Box style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)', padding: 'var(--space-1) var(--space-2)' }}>
        <VideoFormatList formats={filteredFormats} selectedIndex={selectedIndex} focusArea={focusArea} />
      </Box>
    </ScrollArea>
  ) : null
