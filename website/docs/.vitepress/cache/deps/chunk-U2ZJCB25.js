...
var boundingBox = function boundingBox2(obj) {
  return (
    plainObject(obj) && number$1(obj.x1) && number$1(obj.x2) && number$1(obj.y1) && number$1(obj.y2)
  )
}
...
var pnow =
  performance$1 && performance$1.now
    ? function () {
...
function rotatePoint(x2, y2, centerX, centerY, angleDegrees) {
  ...
  var rotatedX =
    Math.cos(angleRadians) * (x2 - centerX) - Math.sin(angleRadians) * (y2 - centerY) + centerX
  var rotatedY =
    Math.sin(angleRadians) * (x2 - centerX) + Math.cos(angleRadians) * (y2 - centerY) + centerY
  return {
    x: rotatedX,
    y: rotatedY,
  }
...
var defineSearch = function defineSearch2(params) {
  ...
  return function searchFn(roots, fn3, directed) {
    ...
    var _this$byGroup = this.byGroup(),
      nodes3 = _this$byGroup.nodes,
      edges3 = _this$byGroup.edges
    ...
    ;(function () {
      ...
      heapify = function (array3, cmp) {
        ...
        _ref1 = function () {
          _results1 = []
          for (
            var _j = 0, _ref = floor(array3.length / 2);
      ...
      nsmallest = function (array3, n, cmp) {
        ...
        _results = []
        for (
          _j = 0, _ref1 = min4(n, array3.length);
...
var elesfn$u = {
  dijkstra: function dijkstra(options2) {
    ...
    var _dijkstraDefaults = dijkstraDefaults(options2),
      root = _dijkstraDefaults.root,
      weight8 = _dijkstraDefaults.weight,
...
var elesfn$s = {
  ...
  aStar: function aStar(options2) {
    var cy = this.cy()
    var _aStarDefaults = aStarDefaults(options2),
      root = _aStarDefaults.root,
      goal = _aStarDefaults.goal,
      heuristic2 = _aStarDefaults.heuristic,
      directed = _aStarDefaults.directed,
      weight8 = _aStarDefaults.weight
...
var elesfn$r = {
  ...
  floydWarshall: function floydWarshall(options2) {
    var cy = this.cy()
    var _floydWarshallDefault = floydWarshallDefaults(options2),
      weight8 = _floydWarshallDefault.weight,
...
var elesfn$q = {
  ...
  bellmanFord: function bellmanFord(options2) {
    var _this = this
    var _bellmanFordDefaults = bellmanFordDefaults(options2),
      weight8 = _bellmanFordDefaults.weight,
...
var hypot =
  (_Math$hypot = Math.hypot) !== null && _Math$hypot !== void 0
    ? _Math$hypot
    : function (x2, y2) {
...
var sqdistToQuadraticBezier = function sqdistToQuadraticBezier2(x2, y2, x1, y1, x22, y22, x3, y3) {
  ...
  for (var i = 0; i < params.length; i++) {
    curX =
      Math.pow(1 - params[i], 2) * x1 +
      2 * (1 - params[i]) * params[i] * x22 +
      params[i] * params[i] * x3
    curY =
      Math.pow(1 - params[i], 2) * y1 +
      2 * (1 - params[i]) * params[i] * y22 +
...
var pointInsidePolygon = function pointInsidePolygon2(
  ...
  for (var i = 0; i < transformedPoints.length / 2; i++) {
    transformedPoints[i * 2] =
      (width2 / 2) * (basePoints[i * 2] * cos2 - basePoints[i * 2 + 1] * sin2)
    transformedPoints[i * 2 + 1] =
      (height2 / 2) * (basePoints[i * 2 + 1] * cos2 + basePoints[i * 2] * sin2)
...
var joinLines = function joinLines2(lineSet) {
  ...
  for (var i = 0; i < lineSet.length / 4; i++) {
    ...
    var intersection2 = finiteLinesIntersect(
      currentLineStartX,
      currentLineStartY,
      currentLineEndX,
      currentLineEndY,
...
var roundPolygonIntersectLine = function roundPolygonIntersectLine2(
  ...
  corners.forEach(function (corner, i2) {
    ...
    intersection2 = intersectLineCircle(
      x2,
      y2,
      centerX,
      centerY,
  ...
  for (var i = 0; i < lines.length / 4; i++) {
    intersection2 = finiteLinesIntersect(
      x2,
      y2,
      centerX,
      centerY,
  ...
  if (intersections.length > 2) {
    ...
    var lowestSquaredDistance =
      Math.pow(lowestIntersection[0] - x2, 2) + Math.pow(lowestIntersection[1] - y2, 2)
    for (var _i4 = 1; _i4 < intersections.length / 2; _i4++) {
      var squaredDistance =
        Math.pow(intersections[_i4 * 2] - x2, 2) + Math.pow(intersections[_i4 * 2 + 1] - y2, 2)
...
var generateUnitNgonPointsFitToSquare = function generateUnitNgonPointsFitToSquare2(
  sides,
  rotationRadians
) {
...
function satPolygonIntersection(poly1, poly2) {
  ...
  function project(polygon2, axis2) {
    var min4 = Infinity
    var max5 = -Infinity
    var _iterator = _createForOfIteratorHelper(polygon2),
      _step
    try {
...
var elesfn$o = {
  pageRank: function pageRank(options2) {
    var _pageRankDefaults = pageRankDefaults(options2),
      dampingFactor = _pageRankDefaults.dampingFactor,
      precision = _pageRankDefaults.precision,
      iterations = _pageRankDefaults.iterations,
      weight8 = _pageRankDefaults.weight
    var cy = this._private.cy
    var _this$byGroup = this.byGroup(),
      nodes3 = _this$byGroup.nodes,
      edges3 = _this$byGroup.edges
...
var elesfn$m = {
  closenessCentralityNormalized: function closenessCentralityNormalized(options2) {
    var _defaults = defaults$e(options2),
      harmonic = _defaults.harmonic,
      weight8 = _defaults.weight,
  ...
  closenessCentrality: function closenessCentrality(options2) {
    var _defaults2 = defaults$e(options2),
      root = _defaults2.root,
      weight8 = _defaults2.weight,
...
var elesfn$l = {
  ...
  betweennessCentrality: function betweennessCentrality(options2) {
    var _defaults = defaults$d(options2),
      directed = _defaults.directed,
      weight8 = _defaults.weight
...
var mergeClosest = function mergeClosest2(clusters, index, dists, mins, opts) {
  ...
  if (
    (opts.mode === 'threshold' && min4 >= opts.threshold) ||
  ...
  for (var _i = 0; _i < clusters.length; _i++) {
    ...
    if (c1.key === cur.key) {
      ...
      dist3 =
        (dists[c1.key][cur.key] * c1.size + dists[c2.key][cur.key] * c2.size) / (c1.size + c2.size)
    } else {
...
var affinityPropagation = function affinityPropagation2(options2) {
  ...
  for (iter = 0; iter < opts.maxIterations; iter++) {
    for (var _i8 = 0; _i8 < n; _i8++) {
      ...
      R[_i8 * n + maxI] =
        (1 - opts.damping) * (S[_i8 * n + maxI] - max22) + opts.damping * old[maxI]
    }
...
var elesfn$k = {
  hierholzer: function hierholzer(options2) {
    ...
    var _hierholzerDefaults = hierholzerDefaults(options2),
...
var hopcroftTarjanBiconnected = function hopcroftTarjanBiconnected2() {
  ...
  var cutVertices = Object.keys(nodes3)
    .filter(function (id3) {
    ...
    try {
      then2.call(
        ...
        function (y2) {
          if (resolved) return
          resolved = true
          if (y2 === x2) promise4.reject(new TypeError('circular thenable chain'))
...
function require_isKey() {
  ...
  function isKey(value, object3) {
    ...
    return (
      ...
      (object3 != null && value in Object(object3))
    )
  }
  _isKey = isKey
  return _isKey
...
function requireMemoize() {
  ...
  function memoize3(func, resolver3) {
    ...
    var memoized = function () {
      var args = arguments,
        key = resolver3 ? resolver3.apply(this, args) : args[0],
        cache3 = memoized.cache
      if (cache3.has(key)) {
...
function require_arrayMap() {
  ...
  function arrayMap(array3, iteratee) {
    var index = -1,
      length = array3 == null ? 0 : array3.length,
      result = Array(length)
    while (++index < length) {
...
function require_assignValue() {
  ...
  function assignValue(object3, key, value) {
    ...
    if (
      !(hasOwnProperty.call(object3, key) && eq2(objValue, value)) ||
...
var toString = function toString2() {
  ...
  var checkToString = function checkToString2(check, subject) {
    ...
    switch (type) {
      ...
      case Type.CHILD: {
        return (
          queryToString(check.parent, subject) + space('>') + queryToString(check.child, subject)
        )
      }
      case Type.ANCESTOR:
      case Type.DESCENDANT: {
        return (
          queryToString(check.ancestor, subject) + ' ' + queryToString(check.descendant, subject)
        )
      }
...
elesfn$b.updateCompoundBounds = function () {
  ...
  function update(parent4) {
        ...
        switch (relativeTo) {
          ...
          case 'min':
            return width2 > 0 && height2 > 0
              ? width2 > height2
                ? paddingObject.pfValue * height2
                : paddingObject.pfValue * width2
              : 0
          case 'max':
            return width2 > 0 && height2 > 0
              ? width2 > height2
    ...
    _p2.autoPadding = computePaddingValues(
      bb.w,
      bb.h,
      parent4.pstyle('padding'),
      parent4.pstyle('padding-relative-to').value
    )
...
var updateBoundsFromMiter = function updateBoundsFromMiter2(
  bounds2,
  ele,
  opacity,
  expansionSize,
...
var boundingBoxImpl = function boundingBoxImpl2(ele, options2) {
  ...
  var displayed =
    !styleEnabled2 ||
    (isDisplayed(ele) && (!isEdge2 || (isDisplayed(ele.source()) && isDisplayed(ele.target()))))
  if (displayed) {
  ...
  if (!usingDefOpts) {
    ...
    if (opts.includeLabels) {
      if (
        ...
        (!isEdge2 || (opts.includeSourceLabels && opts.includeTargetLabels))
      ) {
...
p.on = p.addListener = function (events, qualifier, callback, conf, confOverrides) {
  forEachEvent(
    this,
    function (self2, event3, type, namespace, qualifier2, callback2, conf2) {
      if (fn$6(callback2)) {
...
p.removeListener = p.off = function (events, qualifier, callback, conf) {
  ...
  var _loop = function _loop2(i2) {
    ...
    forEachEvent(
      _this,
      function (self2, event3, type, namespace, qualifier2, callback2) {
        if (
          (listener.type === type || events === '*') &&
          ((!namespace && listener.namespace !== '.*') || listener.namespace === namespace) &&
          (!qualifier2 || self2.qualifierCompare(listener.qualifier, qualifier2)) &&
...
var emitterOptions$1 = {
  ...
  eventMatches: function eventMatches2(ele, listener, eventObj) {
    ...
    if (selector != null) {
      return (
        ele !== eventObj.target && element(eventObj.target) && selector.matches(eventObj.target)
      )
    }
    return true
  },
...
var defineSymbolIterator = function defineSymbolIterator2() {
  ...
  var isIteratorSupported =
    (typeof Symbol === 'undefined' ? 'undefined' : _typeof(Symbol)) != typeofUndef &&
    _typeof(Symbol.iterator) != typeofUndef
...
var elesfn$5 = {
  ...
  layoutPositions: function layoutPositions(layout4, options2, fn3) {
    ...
    if (options2.animate) {
      ...
      Promise$1.all(
        layout4.animations.map(function (ani2) {
          return ani2.promise()
        })
      ).then(function () {
  ...
  return function () {
    ...
    if (ele) {
      ...
      if (ele.isNode()) {
        ...
        return (
          edgeOkViaNode(src) &&
          (!hasCompoundNodes2 || checkCompound(src, edgeOkViaNode)) &&
          (src === tgt ||
            (edgeOkViaNode(tgt) && (!hasCompoundNodes2 || checkCompound(tgt, edgeOkViaNode))))
        )
      }
    }
  }
  ...
  return function edgesWithImpl(otherNodes) {
    ...
    for (var h = 0; h < otherNodes.length; h++) {
      ...
      for (var i = 0; i < edges3.length; i++) {
        ...
        var thisToOther =
          this.hasElementWithId(edgeData.source) && otherNodes.hasElementWithId(edgeData.target)
        var otherToThis =
          otherNodes.hasElementWithId(edgeData.source) && this.hasElementWithId(edgeData.target)
  ...
  if (plainObject(obj)) {
    ...
    if (obj.data) {
      ...
      if (ele.isEdge()) {
        ...
        if (
          newParentValSpecd &&
          (parent4 != null || _data2.parent != null) &&
          parent4 != _data2.parent
        ) {
          if (parent4 === void 0) {
            parent4 = null
          }
...
elesfn$1.restore = function () {
  ...
  for (var _i4 = 0; _i4 < nodes3.length; _i4++) {
    ...
    if (specifiedParent || node._private.parent) {
      var parent4 = node._private.parent
...
function generateCubicBezier(mX1, mY1, mX2, mY2) {
  ...
  function getTForX(aX) {
    ...
    var dist3 =
      ...
      guessForT = intervalStart + dist3 * kSampleStepSize,
...
styfn$8.updateStyleHints = function (ele) {
  ...
  if (isNode2) {
    ...
    var nodeKeys = [nodeBody, nodeBorder, nodeOutline, backgroundImage, compound, pie, stripe]
      .filter(function (k) {
        return k != null
      })
    ...
    _p.hasStripe =
      stripe != null && stripe[0] !== DEFAULT_HASH_SEED && stripe[1] !== DEFAULT_HASH_SEED_ALT
  }
...
styfn$3.appendFromString = function (string3) {
  ...
  for (;;) {
    ...
    if (selectorStr !== 'core') {
      ...
      if (selector.invalid) {
        warn(
          'Skipping parsing of block: Invalid selector found in string stylesheet: ' + selectorStr
        )
...
;(function () {
  ...
  styfn$2.types = {
    ...
    easing: {
      regexes: [
        '^(spring)\\s*\\(\\s*(' + number$12 + ')\\s*,\\s*(' + number$12 + ')\\s*\\)$',
        '^(cubic-bezier)\\s*\\(\\s*(' +
          number$12 +
          ')\\s*,\\s*(' +
          number$12 +
  ...
  var visibility = [
    {
      ...
      triggersBoundsOfParallelEdges: function triggersBoundsOfParallelEdges(
        fromValue,
        toValue,
        ele
      ) {
...
styfn$1.parse = function (name, value, propIsBypass, propIsFlat) {
  ...
  var flatKey =
    propIsFlat === 'mapping' || propIsFlat === true || propIsFlat === false || propIsFlat == null
      ? 'dontcare'
      : propIsFlat
...
styfn$1.parseImpl = function (name, value, propIsBypass, propIsFlat) {
  ...
  if (!valueIsString || propIsFlat || value.length < 7 || value[1] !== 'a');
    ...
    value.length >= 7 &&
    value[0] === 'd' &&
    (data4 = new RegExp(types.data.regex).exec(value))
  ) {
    if (propIsBypass) {
      return false
    }
  ...
  if (type.number) {
    ...
    if (
      (type.min !== void 0 && (value < type.min || (type.strictMin && value === type.min))) ||
      (type.max !== void 0 && (value > type.max || (type.strictMax && value === type.max)))
    ) {
      return null
    }
    var ret = {
...
extend(corefn, {
  ...
  mount: function mount(container2) {
    ...
    cy.invalidateSize()
    cy.initRenderer(
      extend({}, options2, options2.renderer, {
...
BreadthFirstLayout.prototype.run = function () {
  ...
  var distanceY = Math.max(
    ...
    depthsLen === 1
      ...
      : // inside a bounding box, no need for top & bottom padding
        hasBoundingBox
        ? (bb.h - options2.padding * 2 - aveNodeSize.h) / (depthsLen - 1)
        : (bb.h - options2.padding * 2 - aveNodeSize.h) / (depthsLen + 1),
    minDistance
  )
      ...
      var distanceX = Math.max(
        ...
        depthSize === 1
          ...
          : // inside a bounding box, no need for left & right padding
            hasBoundingBox
            ? (bb.w - options2.padding * 2 - aveNodeSize.w) /
              ((options2.grid ? maxDepthSize : depthSize) - 1)
            : (bb.w - options2.padding * 2 - aveNodeSize.w) /
              ((options2.grid ? maxDepthSize : depthSize) + 1),
        minDistance
      )
  ...
  if (Object.keys(rotateDegrees).indexOf(options2.direction) === -1) {
    error(
      "Invalid direction '"
        .concat(options2.direction, "' specified for breadthfirst layout. Valid values are: ")
        .concat(Object.keys(rotateDegrees).join(', '))
    )
  }
...
CircleLayout.prototype.run = function () {
  ...
  var eles = options2.eles
  var clockwise =
    options2.counterclockwise !== void 0 ? !options2.counterclockwise : options2.clockwise
  var nodes3 = eles.nodes().not(':parent')
  if (options2.sort) {
    nodes3 = nodes3.sort(options2.sort)
  }
  var bb = makeBoundingBox(
    options2.boundingBox
      ? options2.boundingBox
      : {
          x1: 0,
          y1: 0,
  ...
  var sweep =
    options2.sweep === void 0 ? 2 * Math.PI - (2 * Math.PI) / nodes3.length : options2.sweep
...
ConcentricLayout.prototype.run = function () {
  ...
  for (var _i3 = 0; _i3 < levels.length; _i3++) {
    ...
    var sweep =
      options2.sweep === void 0 ? 2 * Math.PI - (2 * Math.PI) / level.length : options2.sweep
...
var createLayoutInfo = function createLayoutInfo2(cy, layout4, options2) {
  ...
  for (var i = 0; i < layoutInfo.nodeSize; i++) {
    ...
    tempNode.nodeRepulsion = fn$6(options2.nodeRepulsion)
      ? options2.nodeRepulsion(n)
      : options2.nodeRepulsion
  ...
  for (var i = 0; i < layoutInfo.edgeSize; i++) {
    ...
    var idealLength = fn$6(options2.idealEdgeLength)
      ? options2.idealEdgeLength(e)
      : options2.idealEdgeLength
    var elasticity = fn$6(options2.edgeElasticity)
      ? options2.edgeElasticity(e)
      : options2.edgeElasticity
...
BRp$f.registerArrowShapes = function () {
    ...
    arrowShapes[name] = extend(
      {
        ...
        collide: function collide(x2, y2, size3, angle2, translation, padding) {
          var points = pointsToArr(
            transformPoints3(this.points, size3 + 2 * padding, angle2, translation)
          )
  ...
  defineArrowShape('triangle-tee', {
    ...
    collide: function collide(x2, y2, size3, angle2, translation, edgeWidth, padding) {
      ...
      var teePts = pointsToArr(
        transformPoints3(this.pointsTee, size3 + 2 * padding, angle2, translation)
      )
      var inside =
        pointInsidePolygonPoints(x2, y2, triPts) || pointInsidePolygonPoints(x2, y2, teePts)
      return inside
    },
    draw: function draw(context, size3, angle2, translation, edgeWidth) {
  ...
  defineArrowShape('circle-triangle', {
    ...
    collide: function collide(x2, y2, size3, angle2, translation, edgeWidth, padding) {
      var t = translation
      var circleInside =
        Math.pow(t.x - x2, 2) + Math.pow(t.y - y2, 2) <=
        Math.pow((size3 + 2 * padding) * this.radius, 2)
      var triPts = pointsToArr(
        transformPoints3(this.points, size3 + 2 * padding, angle2, translation)
      )
    ...
    draw: function draw(context, size3, angle2, translation, edgeWidth) {
      ...
      renderer3.arrowShapeImpl(this.name)(
        context,
        triPts,
        translation.x,
        translation.y,
    ...
    spacing: function spacing(edge) {
      return (
        renderer3.getArrowWidth(edge.pstyle('width').pfValue, edge.pstyle('arrow-scale').value) *
        this.radius
      )
    },
  })
  defineArrowShape('triangle-cross', {
    ...
    collide: function collide(x2, y2, size3, angle2, translation, edgeWidth, padding) {
      ...
      var teePts = pointsToArr(
        transformPoints3(
          this.crossLinePts(size3, edgeWidth),
          size3 + 2 * padding,
          angle2,
          translation
        )
    ...
    draw: function draw(context, size3, angle2, translation, edgeWidth) {
      var triPts = transformPoints3(this.points, size3, angle2, translation)
      var crossLinePts = transformPoints3(
        this.crossLinePts(size3, edgeWidth),
        size3,
        angle2,
        translation
...
BRp$e.findNearestElements = function (x2, y2, interactiveElementsOnly, isTouch) {
  ...
  function addEle(ele2, sqDist) {
    ...
    if (ele2.isEdge() && (sqDist == null || sqDist < minSqDist)) {
      if (nearEdge) {
        if (
          nearEdge.pstyle('z-compound-depth').value === ele2.pstyle('z-compound-depth').value &&
          nearEdge.pstyle('z-compound-depth').value === ele2.pstyle('z-compound-depth').value
        ) {
  ...
  function checkNode(node) {
    ...
    var cornerRadius =
      node.pstyle('corner-radius').value === 'auto' ? 'auto' : node.pstyle('corner-radius').pfValue
  ...
  function checkEdge(edge) {
    ...
    if (rs.edgeType === 'segments' || rs.edgeType === 'straight' || rs.edgeType === 'haystack') {
      ...
      for (var i2 = 0; i2 + 3 < pts2.length; i2 += 2) {
        if (
          inLineVicinity(x2, y2, pts2[i2], pts2[i2 + 1], pts2[i2 + 2], pts2[i2 + 3], width22) &&
          widthSq >
    ...
    if (ele.isNode()) {
      ...
      if (edgeBoxSelectMode === 'contain') {
        ...
        if (
          ...
          rs.endX != null &&
          rs.endY != null &&
          (inBoundingBox(boxBb, rs.startX, rs.startY) || inBoundingBox(boxBb, rs.endX, rs.endY))
        ) {
          box.push(edge)
          _selected = true
...
var calcCornerArc = function calcCornerArc2(
  previousPoint,
  currentPoint,
  nextPoint,
  radiusMax,
...
BRp$c.findSegmentsPoints = function (edge, pairInfo) {
  ...
  for (var s = 0; s < segmentsN; s++) {
    ...
    rs.isArcRadius.push(
      (segmentTs.pfValue[s] !== void 0 ? segmentTs.pfValue[s] : lastRadiusType) === 'arc-radius'
    )
  }
}
...
BRp$c.findBezierPoints = function (edge, pairInfo, i, edgeIsUnbundled, edgeIsSwapped) {
  ...
  var bezierN =
    ctrlptDists && ctrlptWs ? Math.min(ctrlptDists.value.length, ctrlptWs.value.length) : 1
  var ctrlptDist = ctrlptDists ? ctrlptDists.pfValue[0] : void 0
...
BRp$c.tryToCorrectInvalidPoints = function (edge, pairInfo) {
  ...
  if (rs.edgeType === 'bezier') {
    ...
    var arrowW =
      this.getArrowWidth(edge.pstyle('width').pfValue, edge.pstyle('arrow-scale').value) *
...
BRp$c.checkForInvalidEdgeWarning = function (edge) {
  ...
  if (
    rs.nodesOverlap ||
    (number$1(rs.startX) && number$1(rs.startY) && number$1(rs.endX) && number$1(rs.endY))
  ) {
    rs.loggedErr = false
  } else {
    if (!rs.loggedErr) {
...
BRp$c.findEdgeControlPoints = function (edges3) {
  ...
  var _loop = function _loop2() {
    var _pairIds$p = pairIds[p2],
      pairId2 = _pairIds$p.pairId,
      edgeIsUnbundled2 = _pairIds$p.edgeIsUnbundled
    ...
    var srcCornerRadius = (pairInfo.srcCornerRadius =
      src2.pstyle('corner-radius').value === 'auto' ? 'auto' : src2.pstyle('corner-radius').pfValue)
    var tgtCornerRadius = (pairInfo.tgtCornerRadius =
      tgt2.pstyle('corner-radius').value === 'auto' ? 'auto' : tgt2.pstyle('corner-radius').pfValue)
    ...
    for (var _i2 = 0; _i2 < pairInfo.eles.length; _i2++) {
      ...
      var _edgeIsUnbundled =
        ...
        endsWith(_curveStyle, 'segments') ||
        endsWith(_curveStyle, 'taxi')
      ...
      if (
        hasCompounds &&
        (src2.isParent() || src2.isChild() || tgt2.isParent() || tgt2.isChild()) &&
...
BRp$b.findEndpoints = function (edge) {
  ...
  var tgtManEndptPt =
    (_ref =
      (tgtManEndpt === null ||
      tgtManEndpt === void 0 ||
      (_tgtManEndpt$pfValue = tgtManEndpt.pfValue) === null ||
      _tgtManEndpt$pfValue === void 0
        ? void 0
        : _tgtManEndpt$pfValue.length) === 2
        ? tgtManEndpt.pfValue
        : null) !== null && _ref !== void 0
      ? _ref
      : [0, 0]
  var srcManEndptPt =
    (_ref2 =
      (srcManEndpt === null ||
      srcManEndpt === void 0 ||
      (_srcManEndpt$pfValue = srcManEndpt.pfValue) === null ||
      _srcManEndpt$pfValue === void 0
        ? void 0
        : _srcManEndpt$pfValue.length) === 2
        ? srcManEndpt.pfValue
        : null) !== null && _ref2 !== void 0
      ? _ref2
      : [0, 0]
  if (bezier) {
  ...
  if (tgtManEndptVal === 'inside-to-node') {
    ...
    intersect2 = r.nodeShapes[this.getNodeShape(target)].intersectLine(
      tgtPos.x,
      tgtPos.y,
      target.outerWidth(),
      target.outerHeight(),
    ...
    if (
      ...
      var labelIntersect = polygonIntersectLine(
        p1_i[0],
        p1_i[1],
        [lx - lw2, ly - lh2, lx + lw2, ly - lh2, lx + lw2, ly + lh2, lx - lw2, ly + lh2],
        tgtPos.x,
        tgtPos.y
      )
  ...
  if (srcManEndptVal === 'inside-to-node') {
    ...
    intersect2 = r.nodeShapes[this.getNodeShape(source)].intersectLine(
      srcPos.x,
      srcPos.y,
      source.outerWidth(),
      source.outerHeight(),
...
BRp$7.recalculateRenderedStyle = function (eles, useCache) {
  ...
  for (var i = 0; i < eles.length; i++) {
    ...
    if (ele.isEdge() && ele.isBundledBezier()) {
      if (
        ele.parallelEdges().some(function (ele2) {
...
BRp$3.load = function () {
  ...
  var eventInContainer = function eventInContainer2(e) {
    ...
    for (var i = 0; i < positions2.length; i++) {
      ...
      if (
        x2 <= p2.clientX &&
        p2.clientX <= x2 + width2 &&
        y2 <= p2.clientY &&
        p2.clientY <= y2 + height2
      ) {
...
BRp$2.generatePolygon = function (name, points) {
  return (this.nodeShapes[name] = {
    ...
    miterBounds: function miterBounds(
      centerX,
      centerY,
      width2,
      height2,
...
BRp$2.generateRoundPolygon = function (name, points) {
  return (this.nodeShapes[name] = {
    ...
    name,
    points,
    getOrCreateCorners: function getOrCreateCorners(
      centerX,
      centerY,
      width2,
      height2,
    ...
    draw: function draw(context, centerX, centerY, width2, height2, cornerRadius, rs) {
      this.renderer.nodeShapeImpl(
        ...
        height2,
        this.points,
        this.getOrCreateCorners(centerX, centerY, width2, height2, cornerRadius, rs, 'drawCorners')
      )
    },
    intersectLine: function intersectLine(
      nodeX,
      ...
      return roundPolygonIntersectLine(
        ...
        height2,
        padding,
        this.getOrCreateCorners(nodeX, nodeY, width2, height2, cornerRadius, rs, 'corners')
      )
    },
    checkPoint: function checkPoint(
      x2,
      ...
      return pointInsideRoundPolygon(
        ...
        width2,
        height2,
        this.getOrCreateCorners(centerX, centerY, width2, height2, cornerRadius, rs, 'corners')
      )
    },
  })
}
  ...
  return (this.nodeShapes['round-rectangle'] = this.nodeShapes['roundrectangle'] =
    {
      ...
      checkPoint: function checkPoint(
        ...
        var halfHeight = height2 / 2
        cornerRadius =
          cornerRadius === 'auto' ? getRoundRectangleRadius(width2, height2) : cornerRadius
  ...
  return (this.nodeShapes['cut-rectangle'] = this.nodeShapes['cutrectangle'] =
    {
      ...
      checkPoint: function checkPoint(
        ...
        return (
          pointInsidePolygonPoints(x2, y2, cutTrianglePts.topLeft) ||
          pointInsidePolygonPoints(x2, y2, cutTrianglePts.topRight) ||
          pointInsidePolygonPoints(x2, y2, cutTrianglePts.bottomRight) ||
          pointInsidePolygonPoints(x2, y2, cutTrianglePts.bottomLeft)
        )
      },
    })
}
      ...
      checkPoint: function checkPoint(
        ...
        if (
          checkInEllipse(
            ...
            diam,
            diam,
            centerX + width2 / 2 - cornerRadius,
            centerY + height2 / 2 - cornerRadius,
            padding
          )
        ) {
          return true
...
var ElementTextureCacheLookup = (function () {
  function ElementTextureCacheLookup2(getKey3) {
    var doesEleInvalidateKey =
      arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : falsify
...
CRp$a.drawCachedElementPortion = function (
  context,
  ele,
  eleTxrCache,
  pxRatio,
...
CRp$9.drawEdge = function (context, edge, shiftToOriginWithBb) {
  ...
  var drawLine = function drawLine2() {
    var strokeOpacity =
      arguments.length > 0 && arguments[0] !== void 0 ? arguments[0] : effectiveLineOpacity
  ...
  var drawArrows = function drawArrows2() {
    var arrowOpacity =
      arguments.length > 0 && arguments[0] !== void 0 ? arguments[0] : effectiveArrowOpacity
...
CRp$8.drawInscribedImage = function (context, img, node, index, nodeOpacity) {
  ...
  var nodeTW =
    nodeW +
    (getIndexedStyle(node, 'background-width-relative-to', 'value', index) === 'inner'
      ? 0
      : paddingX2)
  var nodeTH =
    nodeH +
    (getIndexedStyle(node, 'background-height-relative-to', 'value', index) === 'inner'
      ? 0
      : paddingX2)
  var rs = node._private.rscratch
    ...
    if (backgroundOpacity > 0 || (textBorderWidth > 0 && borderOpacity > 0)) {
      ...
      if (doFill) {
        context.fillStyle = 'rgba('
          .concat(textBackgroundColor[0], ',')
          .concat(textBackgroundColor[1], ',')
          .concat(textBackgroundColor[2], ',')
          .concat(backgroundOpacity * parentOpacity, ')')
      }
      if (doStroke) {
        context.strokeStyle = 'rgba('
          .concat(textBorderColor[0], ',')
          .concat(textBorderColor[1], ',')
          .concat(textBorderColor[2], ',')
          .concat(borderOpacity * parentOpacity, ')')
  ...
  var drawOutline = function drawOutline2() {
    if (outlineWidth > 0) {
      ...
      if (shape === 'ellipse') {
        ...
        cornerRadius =
          cornerRadius === 'auto' ? getRoundPolygonRadius(sWidth, sHeight) : cornerRadius
        var halfW = sWidth / 2
        var halfH = sHeight / 2
        ...
        cornerRadius =
          cornerRadius === 'auto' ? getRoundRectangleRadius(sWidth, sHeight) : cornerRadius
        ...
        ['rhomboid', 'right-rhomboid', 'round-tag', 'tag', 'vee'].includes(shape)
      ) {
...
CRp$5.createGradientStyleFor = function (context, shapeStyleName, ele, fill, opacity) {
  ...
  var colors2 = ele.pstyle(shapeStyleName + '-gradient-stop-colors').value,
    positions2 = ele.pstyle(shapeStyleName + '-gradient-stop-positions').pfValue
  if (fill === 'radial-gradient') {
    if (ele.isEdge()) {
      ...
      var pos = usePaths
          ...
          : ele.position(),
        width2 = ele.paddedWidth(),
        height2 = ele.paddedHeight()
  ...
  for (var i = 0; i < length; i++) {
    gradientStyle.addColorStop(
      hasPositions ? positions2[i] : i / (length - 1),
      'rgba(' + colors2[i][0] + ',' + colors2[i][1] + ',' + colors2[i][2] + ',' + opacity + ')'
    )
  }
...
CRp$5.matchCanvasSize = function (container2) {
  ...
  if (
    container2 === r.data.bufferCanvases[r.MOTIONBLUR_BUFFER_NODE] ||
    container2 === r.data.bufferCanvases[r.MOTIONBLUR_BUFFER_DRAG]
  ) {
...
CRp$5.render = function (options2) {
  ...
  function setContextTransform2(context2, clear2) {
    ...
    if (
      ...
      (context2 === data4.bufferContexts[r.MOTIONBLUR_BUFFER_NODE] ||
        context2 === data4.bufferContexts[r.MOTIONBLUR_BUFFER_DRAG])
    ) {
      ePan = {
        x: pan2.x * mbPxRatio,
        y: pan2.y * mbPxRatio,
  ...
  var needMbClear = []
  needMbClear[r.NODE] =
    (!needDraw[r.NODE] && motionBlur && !r.clearedForMotionBlur[r.NODE]) || r.clearingMotionBlur
  if (needMbClear[r.NODE]) {
    r.clearedForMotionBlur[r.NODE] = true
  }
  needMbClear[r.DRAG] =
    (!needDraw[r.DRAG] && motionBlur && !r.clearedForMotionBlur[r.DRAG]) || r.clearingMotionBlur
  ...
  if (needDraw[r.NODE] || drawAllLayers || drawOnlyNodeLayer || needMbClear[r.NODE]) {
    ...
    var context =
      forcedContext ||
      (useBuffer ? r.data.bufferContexts[r.MOTIONBLUR_BUFFER_NODE] : data4.contexts[r.NODE])
  ...
  if (!drawOnlyNodeLayer && (needDraw[r.DRAG] || drawAllLayers || needMbClear[r.DRAG])) {
    ...
    var context =
      forcedContext ||
      (useBuffer ? r.data.bufferContexts[r.MOTIONBLUR_BUFFER_DRAG] : data4.contexts[r.DRAG])
...
CRp$5.drawSelectionRectangle = function (options2, setContextTransform2) {
  ...
  if (r.showFps || (!drawOnlyNodeLayer && needDraw[r.SELECT_BOX] && !drawAllLayers)) {
    ...
    if (r.selection[4] == 1 && (r.hoverData.selecting || r.touchData.selecting)) {
      ...
      context.fillStyle =
        'rgba(' +
        style3.core('selection-box-color').value[0] +
        ',' +
        style3.core('selection-box-color').value[1] +
        ',' +
        style3.core('selection-box-color').value[2] +
        ',' +
        style3.core('selection-box-opacity').value +
        ')'
      context.fillRect(
        r.selection[0],
        r.selection[1],
      ...
      if (borderWidth > 0) {
        context.strokeStyle =
          'rgba(' +
          style3.core('selection-box-border-color').value[0] +
          ',' +
          style3.core('selection-box-border-color').value[1] +
          ',' +
          style3.core('selection-box-border-color').value[2] +
          ',' +
          style3.core('selection-box-opacity').value +
          ')'
        context.strokeRect(
    ...
    if (data4.bgActivePosistion && !r.hoverData.selecting) {
      ...
      context.fillStyle =
        'rgba(' +
        style3.core('active-bg-color').value[0] +
        ',' +
        style3.core('active-bg-color').value[1] +
        ',' +
        style3.core('active-bg-color').value[2] +
        ',' +
        style3.core('active-bg-opacity').value +
        ')'
      context.beginPath()
    ...
    },
    {
      ...
      value: function _createAtlas() {
        var r = this.r,
          ...
          texRows = this.texRows,
          createTextureCanvas2 = this.createTextureCanvas
...
function intersection(set1, set2) {
  if (set1.intersection) return set1.intersection(set2)
    ...
    return new Set(
      _toConsumableArray(set1).filter(function (x2) {
        return set2.has(x2)
      })
    )
}
        ...
        try {
          for (_iterator6.s(); !(_step6 = _iterator6.n()).done; ) {
            var _step6$value = _slicedToArray(_step6.value, 2),
              name = _step6$value[0],
              collection4 = _step6$value[1]
            var _collection$getCounts = collection4.getCounts(),
  ...
  return _createClass(ElementDrawingWebGL2, [
      ...
      value: function _createShaderProgram(renderTarget) {
        ...
        var vertexShaderSource =
          '#version 300 es\n      precision highp float;\n\n      uniform mat3 uPanZoomMatrix;\n      uniform int  uAtlasSize;\n      \n      // instanced\n      in vec2 aPosition; // a vertex from the unit square\n      \n      in mat3 aTransform; // used to transform verticies, eg into a bounding box\n      in int aVertType; // the type of thing we are rendering\n\n      // the z-index that is output when using picking mode\n      in vec4 aIndex;\n      \n      // For textures\n      in int aAtlasId; // which shader unit/atlas to use\n      in vec4 aTex; // x/y/w/h of texture in atlas\n\n      // for edges\n      in vec4 aPointAPointB;\n      in vec4 aPointCPointD;\n      in vec2 aLineWidth; // also used for node border width\n\n      // simple shapes\n      in vec4 aCornerRadius; // for round-rectangle [top-right, bottom-right, top-left, bottom-left]\n      in vec4 aColor; // also used for edges\n      in vec4 aBorderColor; // aLineWidth is used for border width\n\n      // output values passed to the fragment shader\n      out vec2 vTexCoord;\n      out vec4 vColor;\n      out vec2 vPosition;\n      // flat values are not interpolated\n      flat out int vAtlasId; \n      flat out int vVertType;\n      flat out vec2 vTopRight;\n      flat out vec2 vBotLeft;\n      flat out vec4 vCornerRadius;\n      flat out vec4 vBorderColor;\n      flat out vec2 vBorderWidth;\n      flat out vec4 vIndex;\n      \n      void main(void) {\n        int vid = gl_VertexID;\n        vec2 position = aPosition; // TODO make this a vec3, simplifies some code below\n\n        if(aVertType == '
            ...
            .concat(RECTANGLE, ' || aVertType == ')
            .concat(ELLIPSE, ' \n             || aVertType == ')
            .concat(ROUND_RECTANGLE, ' || aVertType == ')
            .concat(
              BOTTOM_ROUND_RECTANGLE,
          ...
          '#version 300 es\n      precision highp float;\n\n      // declare texture unit for each texture atlas in the batch\n      '
            ...
            .concat(
              idxs
                .map(function (i2) {
                  return 'if(vAtlasId == '
                    .concat(i2, ') outColor = texture(uTexture')
                    .concat(i2, ', vTexCoord);')
                })
                .join('\n	else '),
              '\n        } \n        else if(vVertType == '
            ...
            .concat(RECTANGLE, ' || vVertType == ')
            .concat(ELLIPSE, ' \n          || vVertType == ')
            .concat(ROUND_RECTANGLE, ' || vVertType == ')
            .concat(
              BOTTOM_ROUND_RECTANGLE,
    ...
    },
    {
      ...
      value: function startFrame(panZoomMatrix) {
        var renderTarget =
          arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : RENDER_TARGET.SCREEN
    ...
    },
    {
      ...
      value: function drawNode(node, eleIndex, type) {
        ...
        var indexView = this.indexBuffer.getView(instance)
        indexToVec4(eleIndex, indexView)
        var opacity = this.renderTarget.picking
          ? 1
          : type === 'node-body'
...
CRp$4.initWebgl = function (opts, fns) {
  ...
  opts.webglTexPerBatch = Math.min(
    opts.webglTexPerBatch,
...
function createPanZoomMatrix(r) {
  ...
  var _util$getEffectivePan = getEffectivePanZoom(r),
    pan2 = _util$getEffectivePan.pan,
    zoom2 = _util$getEffectivePan.zoom
...
function setContextTransform(r, context) {
  ...
  var _util$getEffectivePan2 = getEffectivePanZoom(r),
    pan2 = _util$getEffectivePan2.pan,
    zoom2 = _util$getEffectivePan2.zoom
  context.setTransform(1, 0, 0, 1, 0, 0)
...
function getPickingIndexes(r, mX1, mY1, mX2, mY2) {
  ...
  var _util$getEffectivePan3 = getEffectivePanZoom(r),
    pan2 = _util$getEffectivePan3.pan,
    zoom2 = _util$getEffectivePan3.zoom
  {
    var _util$modelToRendered = modelToRenderedPosition2(r, pan2, zoom2, mX1, mY1),
...
function renderWebgl(r, options2, renderTarget) {
  ...
  if (r.webglDebug) {
    ...
    if (compact) {
      console.log(
        'WebGL ('.concat(renderTarget.name, ') - time ').concat(time2, 'ms, ').concat(report)
      )
    } else {
      ...
      try {
        for (_iterator2.s(); !(_step2 = _iterator2.n()).done; ) {
          ...
          console.log(
            '  '
              .concat(info.type, ': ')
              .concat(info.keyCount, ' keys, ')
              .concat(info.atlasCount, ' atlases')
          )
        }
      } catch (err) {
        _iterator2.e(err)
...
CRp$3.drawRoundRectanglePath = function (context, x2, y2, width2, height2, radius2) {
  ...
  var cornerRadius =
    radius2 === 'auto'
      ? getRoundRectangleRadius(width2, height2)
      : Math.min(radius2, halfHeight, halfWidth)
  if (context.beginPath) {
    context.beginPath()
  }
...
CRp.makeOffscreenCanvas = function (width2, height2) {
  var canvas
  if (
    (typeof OffscreenCanvas === 'undefined' ? 'undefined' : _typeof(OffscreenCanvas)) !==
    'undefined'
  ) {
...
;[CRp$b, CRp$a, CRp$9, CRp$8, CRp$7, CRp$6, CRp$5, CRp$4, CRp$3, CRp$2, CRp$1].forEach(
  function (props) {
    extend(CRp, props)
  }
)
...
function setExtension(type, name, registrant) {
  ...
  if (type === 'core') {
    ...
    return error(
      type + ' is an illegal type to be registered, possibly lead to prototype pollutions'
    )
  }
  return setMap({
    map: extensions,
...
*/
//# sourceMappingURL=chunk-U2ZJCB25.js.map
