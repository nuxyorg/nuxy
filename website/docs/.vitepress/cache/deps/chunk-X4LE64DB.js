import { require_dayjs_min } from './chunk-R6XR7RJH.js'
import { __toESM } from './chunk-EQCVQC35.js'
...
var log = {
  trace: __name((..._args) => {}, 'trace'),
  debug: __name((..._args) => {}, 'debug'),
  info: __name((..._args) => {}, 'info'),
  warn: __name((..._args) => {}, 'warn'),
  error: __name((..._args) => {}, 'error'),
  fatal: __name((..._args) => {}, 'fatal'),
}
...
function max(values, valueof) {
  ...
  if (valueof === void 0) {
    ...
    for (let value of values) {
      if (
        (value = valueof(value, ++index2, values)) != null &&
...
var Adder = class {
  ...
  add(x3) {
    ...
    for (let j = 0; j < this._n && j < 32; j++) {
      const y3 = p[j],
        hi = x3 + y3,
        lo = Math.abs(x3) < Math.abs(y3) ? x3 - (hi - y3) : y3 - (hi - x3)
      if (lo) p[i++] = lo
      x3 = hi
    }
    p[i] = x3
...
function ticks(start2, stop, count3) {
  ...
  const reverse2 = stop < start2,
    [i1, i2, inc] = reverse2 ? tickSpec(stop, start2, count3) : tickSpec(start2, stop, count3)
  if (!(i2 >= i1)) return []
  const n = i2 - i1 + 1,
    ticks2 = new Array(n)
  if (reverse2) {
...
function tickStep(start2, stop, count3) {
  ;((stop = +stop), (start2 = +start2), (count3 = +count3))
  const reverse2 = stop < start2,
    inc = reverse2 ? tickIncrement(stop, start2, count3) : tickIncrement(start2, stop, count3)
...
function axis(orient, scale2) {
  var tickArguments = [],
    ...
    x3 = orient === left || orient === right ? 'x' : 'y',
  ...
  function axis2(context) {
    var values =
      ...
      range2 = scale2.range(),
      range0 = +range2[0] + offset,
      range1 = +range2[range2.length - 1] + offset,
...
function select_default(select) {
  ...
  for (
    var groups2 = this._groups, m = groups2.length, subgroups = new Array(m), j = 0;
    j < m;
    ++j
  ) {
    for (
...
function selectAll_default(select) {
  ...
  for (
    var groups2 = this._groups, m = groups2.length, subgroups = [], parents = [], j = 0;
    j < m;
    ++j
  ) {
...
function data_default(value, key) {
  ...
  for (
    var m = groups2.length, update = new Array(m), enter = new Array(m), exit = new Array(m), j = 0;
    j < m;
    ++j
  ) {
...
function sort_default(compare) {
  ...
  for (
    var groups2 = this._groups, m = groups2.length, sortgroups = new Array(m), j = 0;
    j < m;
    ++j
  ) {
    for (
        ...
        : l === 3
          ...
          : l === 8
            ...
            : l === 4
              ...
              : null)
    : (m = reRgbInteger.exec(format3))
      ? new Rgb(m[1], m[2], m[3], 1)
      : (m = reRgbPercent.exec(format3))
        ? new Rgb((m[1] * 255) / 100, (m[2] * 255) / 100, (m[3] * 255) / 100, 1)
        : (m = reRgbaInteger.exec(format3))
          ? rgba(m[1], m[2], m[3], m[4])
          : (m = reRgbaPercent.exec(format3))
            ? rgba((m[1] * 255) / 100, (m[2] * 255) / 100, (m[3] * 255) / 100, m[4])
            : (m = reHslPercent.exec(format3))
              ? hsla(m[1], m[2] / 100, m[3] / 100, 1)
              : (m = reHslaPercent.exec(format3))
                ? hsla(m[1], m[2] / 100, m[3] / 100, m[4])
                : named.hasOwnProperty(format3)
                  ? rgbn(named[format3])
                  : format3 === 'transparent'
...
function labConvert(o) {
  ...
  var r = rgb2lrgb(o.r),
    g = rgb2lrgb(o.g),
    b = rgb2lrgb(o.b),
    y3 = xyz2lab((0.2225045 * r + 0.7168786 * g + 0.0606169 * b) / Yn),
    x3,
    z
  if (r === g && g === b) x3 = z = y3
  else {
...
define_default(
  ...
  extend(Color, {
    ...
    rgb() {
      var y3 = (this.l + 16) / 116,
        x3 = isNaN(this.a) ? y3 : y3 + this.a / 500,
        z = isNaN(this.b) ? y3 : y3 - this.b / 200
      x3 = Xn * lab2xyz(x3)
      y3 = Yn * lab2xyz(y3)
      z = Zn * lab2xyz(z)
      return new Rgb(
  ...
  extend(Color, {
    ...
    rgb() {
      var h = isNaN(this.h) ? 0 : (this.h + 120) * radians,
        l = +this.l,
        a2 = isNaN(this.s) ? 0 : this.s * l * (1 - l),
        cosh2 = Math.cos(h),
        sinh2 = Math.sin(h)
      return new Rgb(
...
function hue(a2, b) {
  var d = b - a2
  return d
    ? linear(a2, d > 180 || d < -180 ? d - 360 * Math.round(d / 360) : d)
...
function formatDecimal_default(x3) {
  return Math.abs((x3 = Math.round(x3))) >= 1e21
    ? x3.toLocaleString('en').replace(/,/g, '')
    : x3.toString(10)
}
function formatDecimalParts(x3, p) {
  if (!isFinite(x3) || x3 === 0) return null
  var i = (x3 = p ? x3.toExponential(p - 1) : x3.toExponential()).indexOf('e'),
    coefficient = x3.slice(0, i)
  return [
  ...
  return i === n
    ...
    : i > n
      ...
      : i > 0
        ...
        : '0.' + new Array(1 - i).join('0') + formatDecimalParts(x3, Math.max(0, p + i - 1))[0]
}
        ...
        valuePrefix =
          (valueNegative
            ...
            : sign3 === '-' || sign3 === '('
              ...
              : sign3) + valuePrefix
        valueSuffix =
          (type2 === 's' && !isNaN(value) && prefixExponent !== void 0
        ...
        if (maybeSuffix) {
          ...
          while (++i < n) {
            if (((c3 = value.charCodeAt(i)), 48 > c3 || c3 > 57)) {
              valueSuffix =
                (c3 === 46 ? decimal + value.slice(i + 1) : value.slice(i)) + valueSuffix
...
function dice_default(parent, x02, y0, x12, y1) {
  var nodes = parent.children,
    ...
    i = -1,
    n = nodes.length,
    k2 = parent.value && (x12 - x02) / parent.value
  while (++i < n) {
...
function slice_default(parent, x02, y0, x12, y1) {
  var nodes = parent.children,
    ...
    i = -1,
    n = nodes.length,
    k2 = parent.value && (y1 - y0) / parent.value
  while (++i < n) {
...
function treemap_default() {
  ...
  treemap.paddingInner = function (x3) {
    return arguments.length
      ? ((paddingInner = typeof x3 === 'function' ? x3 : constant_default3(+x3)), treemap)
      : paddingInner
  }
  treemap.paddingOuter = function (x3) {
    return arguments.length
      ? treemap.paddingTop(x3).paddingRight(x3).paddingBottom(x3).paddingLeft(x3)
      : treemap.paddingTop()
  }
  treemap.paddingTop = function (x3) {
    return arguments.length
      ? ((paddingTop = typeof x3 === 'function' ? x3 : constant_default3(+x3)), treemap)
      : paddingTop
  }
  treemap.paddingRight = function (x3) {
    return arguments.length
      ? ((paddingRight = typeof x3 === 'function' ? x3 : constant_default3(+x3)), treemap)
      : paddingRight
  }
  treemap.paddingBottom = function (x3) {
    return arguments.length
      ? ((paddingBottom = typeof x3 === 'function' ? x3 : constant_default3(+x3)), treemap)
      : paddingBottom
  }
  treemap.paddingLeft = function (x3) {
    return arguments.length
      ? ((paddingLeft = typeof x3 === 'function' ? x3 : constant_default3(+x3)), treemap)
      : paddingLeft
  }
  return treemap
}
...
function transformer() {
  ...
  function scale2(x3) {
    return x3 == null || isNaN((x3 = +x3))
      ? unknown
      : (output || (output = piecewise2(domain.map(transform2), range2, interpolate)))(
          transform2(clamp(x3))
        )
  }
  scale2.invert = function (y3) {
    return clamp(
      untransform(
        (input || (input = piecewise2(range2, domain.map(transform2), number_default)))(y3)
      )
    )
  }
  scale2.domain = function (_) {
...
function timeInterval(floori, offseti, count3, field) {
  ...
  interval2.range = (start2, stop, step) => {
    ...
    let previous
    do
      (range2.push((previous = /* @__PURE__ */ new Date(+start2))),
        offseti(start2, step),
        floori(start2))
...
function calendar(ticks2, tickInterval, year, month, week, day, hour, minute, second2, format3) {
  ...
  var formatMillisecond = format3('.%L'),
    ...
    formatMonth = format3('%B'),
    formatYear3 = format3('%Y')
  ...
  scale2.copy = function () {
    return copy(
      scale2,
      calendar(ticks2, tickInterval, year, month, week, day, hour, minute, second2, format3)
    )
  }
  return scale2
}
...
var Path = class {
  ...
  arcTo(x12, y1, x22, y22, r) {
    ...
    if (this._x1 === null) {
      ...
      let x20 = x22 - x02,
        ...
        l21 = Math.sqrt(l21_2),
        l01 = Math.sqrt(l01_2),
        l = r * Math.tan((pi2 - Math.acos((l21_2 + l01_2 - l20_2) / (2 * l21 * l01))) / 2),
        t01 = l / l01,
        t21 = l / l21
      ...
      this
        ._append`A${r},${r},0,0,${+(y01 * x20 > x01 * y20)},${(this._x1 = x12 + t21 * x21)},${(this._y1 = y1 + t21 * y21)}`
    }
  }
  arc(x3, y3, r, a0, a1, ccw) {
    ...
    if (da2 > tauEpsilon) {
      this
        ._append`A${r},${r},0,1,${cw},${x3 - dx},${y3 - dy}A${r},${r},0,1,${cw},${(this._x1 = x02)},${(this._y1 = y0)}`
    } else if (da2 > epsilon4) {
      this
        ._append`A${r},${r},0,${+(da2 >= pi2)},${cw},${(this._x1 = x3 + r * Math.cos(a1))},${(this._y1 = y3 + r * Math.sin(a1))}`
    }
  }
  rect(x3, y3, w, h) {
    this
      ._append`M${(this._x0 = this._x1 = +x3)},${(this._y0 = this._y1 = +y3)}h${(w = +w)}v${+h}h${-w}Z`
  }
  toString() {
    return this._
  }
...
function cornerTangents(x02, y0, x12, y1, r1, rc, cw) {
  var x01 = x02 - x12,
    y01 = y0 - y1,
    lo = (cw ? rc : -rc) / sqrt2(x01 * x01 + y01 * y01),
    ox = lo * y01,
    oy = -lo * x01,
    x11 = x02 + ox,
    y11 = y0 + oy,
    ...
    r = r1 - rc,
    D3 = x11 * y10 - x10 * y11,
    d = (dy < 0 ? -1 : 1) * sqrt2(max2(0, r * r * d2 - D3 * D3)),
    cx0 = (D3 * dy - dx * d) / d2,
    cy0 = (-D3 * dx - dy * d) / d2,
    cx1 = (D3 * dy + dx * d) / d2,
    cy1 = (-D3 * dx + dy * d) / d2,
...
function arc_default() {
  var innerRadius = arcInnerRadius,
    ...
    cornerRadius = constant_default4(0),
    padRadius = null,
  ...
  function arc() {
    ...
    if (!(r1 > epsilon3)) context.moveTo(0, 0)
      ...
      var a01 = a0,
        ...
        rp =
          ap > epsilon3 &&
          (padRadius ? +padRadius.apply(this, arguments) : sqrt2(r0 * r0 + r1 * r1)),
        rc = min2(abs(r1 - r0) / 2, +cornerRadius.apply(this, arguments)),
        rc0 = rc,
        rc1 = rc,
        t03,
        t13
          ...
          if ((oc = intersect(x01, y01, x00, y00, x11, y11, x10, y10))) {
            var ax = x01 - oc[0],
              ...
              kc =
                ...
                sin(
                  acos(
                    (ax * bx + ay * by) / (sqrt2(ax * ax + ay * ay) * sqrt2(bx * bx + by * by))
                  ) / 2
                ),
              lc = sqrt2(oc[0] * oc[0] + oc[1] * oc[1])
      ...
      if (!(da1 > epsilon3)) context.moveTo(x01, y01)
        ...
        if (rc1 < rc)
          ...
          context.arc(
            ...
            0,
            r1,
            atan2(t03.cy + t03.y11, t03.cx + t03.x11),
            atan2(t13.cy + t13.y11, t13.cx + t13.x11),
            !cw
          )
  ...
  arc.innerRadius = function (_) {
    return arguments.length
      ? ((innerRadius = typeof _ === 'function' ? _ : constant_default4(+_)), arc)
      : innerRadius
  }
  arc.outerRadius = function (_) {
    return arguments.length
      ? ((outerRadius = typeof _ === 'function' ? _ : constant_default4(+_)), arc)
      : outerRadius
  }
  arc.cornerRadius = function (_) {
    return arguments.length
      ? ((cornerRadius = typeof _ === 'function' ? _ : constant_default4(+_)), arc)
      : cornerRadius
  }
  arc.padRadius = function (_) {
    return arguments.length
      ? ((padRadius = _ == null ? null : typeof _ === 'function' ? _ : constant_default4(+_)), arc)
      : padRadius
  }
  arc.startAngle = function (_) {
    return arguments.length
      ? ((startAngle = typeof _ === 'function' ? _ : constant_default4(+_)), arc)
      : startAngle
  }
  arc.endAngle = function (_) {
    return arguments.length
      ? ((endAngle = typeof _ === 'function' ? _ : constant_default4(+_)), arc)
      : endAngle
  }
  arc.padAngle = function (_) {
    return arguments.length
      ? ((padAngle = typeof _ === 'function' ? _ : constant_default4(+_)), arc)
      : padAngle
  }
...
function line_default(x3, y3) {
  var defined = constant_default4(true),
    context = null,
  ...
  line.x = function (_) {
    return arguments.length
      ? ((x3 = typeof _ === 'function' ? _ : constant_default4(+_)), line)
      : x3
  }
  line.y = function (_) {
    return arguments.length
      ? ((y3 = typeof _ === 'function' ? _ : constant_default4(+_)), line)
      : y3
  }
  line.defined = function (_) {
    return arguments.length
      ? ((defined = typeof _ === 'function' ? _ : constant_default4(!!_)), line)
      : defined
  }
...
function pie_default() {
  var value = identity_default3,
    sortValues = descending_default,
    sort2 = null,
    startAngle = constant_default4(0),
    endAngle = constant_default4(tau),
    padAngle = constant_default4(0)
  function pie(data) {
    var i,
      n = (data = array_default3(data)).length,
      j,
      ...
      da2 = Math.min(tau, Math.max(-tau, endAngle.apply(this, arguments) - a0)),
      a1,
  ...
  pie.value = function (_) {
    return arguments.length
      ? ((value = typeof _ === 'function' ? _ : constant_default4(+_)), pie)
      : value
  }
  ...
  pie.startAngle = function (_) {
    return arguments.length
      ? ((startAngle = typeof _ === 'function' ? _ : constant_default4(+_)), pie)
      : startAngle
  }
  pie.endAngle = function (_) {
    return arguments.length
      ? ((endAngle = typeof _ === 'function' ? _ : constant_default4(+_)), pie)
      : endAngle
  }
  pie.padAngle = function (_) {
    return arguments.length
      ? ((padAngle = typeof _ === 'function' ? _ : constant_default4(+_)), pie)
      : padAngle
  }
  return pie
}
...
BasisOpen.prototype = {
  ...
  point: function (x3, y3) {
    ...
    switch (this._point) {
      ...
      case 2:
        this._point = 3
        var x02 = (this._x0 + 4 * this._x1 + x3) / 6,
          y0 = (this._y0 + 4 * this._y1 + y3) / 6
...
function point4(that, x3, y3) {
  ...
  if (that._l01_a > epsilon3) {
    var a2 = 2 * that._l01_2a + 3 * that._l01_a * that._l12_a + that._l12_2a,
...
function DragEvent(
  type2,
  { sourceEvent, subject, target, identifier, active, x: x3, y: y3, dx, dy, dispatch: dispatch2 }
) {
  Object.defineProperties(this, {
...
var elasticInOut = (function custom18(a2, p) {
  ...
  function elasticInOut2(t) {
    return (
      ((t = t * 2 - 1) < 0
        ? a2 * tpmt(-t) * Math.sin((s2 - t) / p)
        : 2 - a2 * tpmt(t) * Math.sin((s2 + t) / p)) / 2
    )
  }
...
function formatDate(date2) {
  ...
  return isNaN(date2)
    ? 'Invalid Date'
    : formatYear2(date2.getUTCFullYear(), 4) +
        '-' +
        pad2(date2.getUTCMonth() + 1, 2) +
        '-' +
        pad2(date2.getUTCDate(), 2) +
        (milliseconds2
          ? 'T' +
            pad2(hours, 2) +
            ':' +
            ...
            pad2(seconds2, 2) +
            '.' +
            pad2(milliseconds2, 3) +
            'Z'
          : seconds2
            ? 'T' + pad2(hours, 2) + ':' + pad2(minutes, 2) + ':' + pad2(seconds2, 2) + 'Z'
            : minutes || hours
              ? 'T' + pad2(hours, 2) + ':' + pad2(minutes, 2) + 'Z'
              : '')
}
...
function find_default2(x3, y3, radius) {
  ...
  while ((q = quads.pop())) {
    ...
    if (node.length) {
      ...
      var dx = x3 - +this._x.call(null, node.data),
        dy = y3 - +this._y.call(null, node.data),
        d2 = dx * dx + dy * dy
      if (d2 < radius) {
...
function visit_default(callback) {
  ...
  while ((q = quads.pop())) {
    if (
      !callback((node = q.node), (x02 = q.x0), (y0 = q.y0), (x12 = q.x1), (y1 = q.y1)) &&
      node.length
    ) {
      var xm = (x02 + x12) / 2,
        ym = (y0 + y1) / 2
...
function longitude(point6) {
  return abs4(point6[0]) <= pi5
    ? point6[0]
    : sign2(point6[0]) * (((abs4(point6[0]) + pi5) % tau6) - pi5)
}
function polygonContains_default(polygon, point6) {
  var lambda = longitude(point6),
    phi2 = point6[1],
    sinPhi = sin3(phi2),
    normal = [sin3(lambda), -cos3(lambda), 0],
    angle = 0,
    winding = 0
  var sum4 = new Adder()
...
function compareIntersection(a2, b) {
  return (
    ((a2 = a2.x)[0] < 0 ? a2[1] - halfPi4 - epsilon7 : halfPi4 - a2[1]) -
    ((b = b.x)[0] < 0 ? b[1] - halfPi4 - epsilon7 : halfPi4 - b[1])
  )
}
...
function clipAntimeridianLine(stream) {
  ...
  return {
    ...
    point: function (lambda1, phi1) {
      var sign1 = lambda1 > 0 ? pi5 : -pi5,
        delta = abs4(lambda1 - lambda0)
...
function clipAntimeridianIntersect(lambda0, phi0, lambda1, phi1) {
  var cosPhi0,
    cosPhi1,
    sinLambda0Lambda1 = sin3(lambda0 - lambda1)
  return abs4(sinLambda0Lambda1) > epsilon7
    ? atan(
        (sin3(phi0) * (cosPhi1 = cos3(phi1)) * sin3(lambda1) -
          sin3(phi1) * (cosPhi0 = cos3(phi0)) * sin3(lambda0)) /
...
function azimuthalInvert(angle) {
  return function (x3, y3) {
    var z = sqrt4(x3 * x3 + y3 * y3),
      c3 = angle(z),
      sc = sin3(c3),
      cc2 = cos3(c3)
    return [atan22(x3 * sc, z * cc2), asin2(z && (y3 * sc) / z)]
  }
}
...
equalEarthRaw.invert = function (x3, y3) {
  ...
  return [(M * x3 * (A1 + 3 * A2 * l2 + l6 * (7 * A3 + 9 * A4 * l2))) / cos3(l), asin2(sin3(l) / M)]
}
  ...
  function randomGamma(k2, theta) {
    ...
    return function () {
      do {
        ...
        u4 >= 1 - 0.0331 * x3 * x3 * x3 * x3 &&
        Math.log(u4) >= 0.5 * x3 * x3 + d * (1 - v2 + Math.log(v2))
      )
...
Transform.prototype = {
  ...
  translate: function (x3, y3) {
    return (x3 === 0) & (y3 === 0)
      ? this
      : new Transform(this.k, this.x + this.k * x3, this.y + this.k * y3)
  },
...
//# sourceMappingURL=chunk-X4LE64DB.js.map
