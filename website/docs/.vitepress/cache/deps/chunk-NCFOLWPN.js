import { configureLabelImages, getSubGraphTitleMargins } from './chunk-EOLJNIOX.js'
import { at } from './chunk-5G2FUE7H.js'
import {
  compileStyles,
  solidStateFill,
  styles2String,
  userNodeOverrides,
} from './chunk-YGFUTLTO.js'
import { createText, getIconSVG } from './chunk-4WLP3MT7.js'
import { calculateTextWidth, decodeEntities, handleUndefinedAttr } from './chunk-N5H3GFM2.js'
import {
  evaluate,
  getConfig,
  getConfig2,
...
var labelHelper = __name(async (parent, node, _classes) => {
  ...
  let cssClasses
  const useHtmlLabels =
    node.useHtmlLabels || evaluate((_a = getConfig2()) == null ? void 0 : _a.htmlLabels)
  if (!_classes) {
...
var insertLabel = __name(async (parent, label, options) => {
  ...
  const text2 = await createText(labelEl, sanitizeText(decodeEntities(label), getConfig2()), {
    ...
    width:
      options.width ||
      ((_b = (_a = getConfig2()) == null ? void 0 : _a.flowchart) == null
        ? void 0
        : _b.wrappingWidth),
...
var createRoundedRectPathD = __name(
  (x, y, totalWidth, totalHeight, radius) =>
    [
      'M',
      x + radius,
      y,
...
async function bowTieRect(parent, node) {
  ...
  const totalHeight = calcTotalHeight(
    (node == null ? void 0 : node.height) ? (node == null ? void 0 : node.height) : bbox.height
  )
  const [rx, ry] = calcEllipseRadius(totalHeight)
  const sagitta = calculateArcSagitta(totalHeight, rx, ry)
  const totalWidth =
    ((node == null ? void 0 : node.width) ? (node == null ? void 0 : node.width) : bbox.width) +
    labelPaddingX * 2 +
    sagitta
...
async function card(parent, node) {
  ...
  const { shapeSvg, bbox } = await labelHelper(parent, node, getNodeClasses(node))
  const w =
    ((node == null ? void 0 : node.width) ?? bbox.width) +
    (node.look === 'neo' ? labelPaddingX * 2 : labelPaddingX + NOTCH_SIZE)
  const h =
    ((node == null ? void 0 : node.height) ?? bbox.height) +
    (node.look === 'neo' ? labelPaddingY * 2 : labelPaddingY)
  const left = 0
  const right = w
  const top = -h
  const bottom = 0
...
function generateCirclePoints2(
  centerX,
  centerY,
  radius,
  numPoints = 100,
...
function generateCirclePoints3(
  centerX,
  centerY,
  radius,
  numPoints = 100,
...
function generateCirclePoints4(
  centerX,
  centerY,
  radius,
  numPoints = 100,
...
async function drawRect(parent, node, options) {
  ...
  const totalWidth = Math.max(
    bbox.width + options.labelPaddingX * 2,
    (node == null ? void 0 : node.width) || 0
  )
  const totalHeight = Math.max(
    bbox.height + options.labelPaddingY * 2,
    (node == null ? void 0 : node.height) || 0
  )
  ...
  if (node.look === 'handDrawn') {
    ...
    const roughNode =
      rx || ry
        ? rc.path(createRoundedRectPathD(x, y, totalWidth, totalHeight, rx || 0), options2)
        : rc.rectangle(x, y, totalWidth, totalHeight, options2)
...
async function doublecircle(parent, node) {
  ...
  const { shapeSvg, bbox } = await labelHelper(parent, node, getNodeClasses(node))
  const outerRadius =
    ((node == null ? void 0 : node.width)
      ? (node == null ? void 0 : node.width) / 2
      : bbox.width / 2) + (labelPadding ?? 0)
...
async function flippedTriangle(parent, node) {
  ...
  const w =
    ((node == null ? void 0 : node.width) ? (node == null ? void 0 : node.width) : bbox.width) +
    (labelPaddingX ?? 0)
  const h = (node == null ? void 0 : node.height)
    ? node == null
      ? void 0
      : node.height
    : w + bbox.height
  ...
  const pathData = createPathFromPoints(points)
  const roughNode = rc.path(pathData, options)
  const flippedTriangle2 = shapeSvg
    .insert(() => roughNode, ':first-child')
  ...
  const w =
    ((node == null ? void 0 : node.width)
      ? node == null
        ? void 0
        : node.width
      : Math.max(minWidth, bbox.width)) +
    paddingX * 2
  const h =
    ((node == null ? void 0 : node.height)
      ? node == null
        ? void 0
        : node.height
      : Math.max(minHeight, bbox.height)) +
    paddingY * 2
  const radius = h / 2
  const { cssStyles } = node
  const rc = at.svg(shapeSvg)
...
async function imageSquare(parent, node, { config: { flowchart } }) {
  ...
  const imageWidth =
    node.constraint === 'on'
      ? (node == null ? void 0 : node.assetHeight)
  ...
  const imageHeight =
    node.constraint === 'on'
      ...
      : ((node == null ? void 0 : node.assetHeight) ?? imageNaturalHeight)
...
async function inv_trapezoid(parent, node) {
  ...
  const { shapeSvg, bbox } = await labelHelper(parent, node, getNodeClasses(node))
  const w = Math.max(
    bbox.width + (labelPaddingX ?? 0) * 2,
    (node == null ? void 0 : node.width) ?? 0
  )
  const h = Math.max(
    bbox.height + (labelPaddingY ?? 0) * 2,
    (node == null ? void 0 : node.height) ?? 0
  )
  const points = [
    { x: 0, y: 0 },
...
async function multiRect(parent, node) {
  ...
  if (node.width || node.height) {
    node.width = Math.max(
      ((node == null ? void 0 : node.width) ?? 0) - labelPaddingX * 2 - 2 * rectOffset2,
      10
    )
    node.height = Math.max(
      ((node == null ? void 0 : node.height) ?? 0) - labelPaddingY * 2 - 2 * rectOffset2,
      10
    )
  }
  const { shapeSvg, bbox, label } = await labelHelper(parent, node, getNodeClasses(node))
...
async function note(parent, node, { config: { themeVariables } }) {
  ...
  const { shapeSvg, bbox, label } = await labelHelper(parent, node, getNodeClasses(node))
  const totalWidth = Math.max(
    bbox.width + (node.padding ?? 0) * 2,
    (node == null ? void 0 : node.width) ?? 0
  )
  const totalHeight = Math.max(
    bbox.height + (node.padding ?? 0) * 2,
    (node == null ? void 0 : node.height) ?? 0
  )
  const x = -totalWidth / 2
  const y = -totalHeight / 2
...
async function rect_left_inv_arrow(parent, node) {
  ...
  const w =
    ((node == null ? void 0 : node.width) ?? bbox.width) +
    (node.look === 'neo' ? labelPaddingX * 2 : labelPaddingX)
  const h =
    ((node == null ? void 0 : node.height) ?? bbox.height) +
    (node.look === 'neo' ? labelPaddingY * 2 : labelPaddingY)
  const x = -w / 2
...
async function slopedRect(parent, node) {
  ...
  if (node.width || node.height) {
    ...
    node.height = Math.max(
      ((node == null ? void 0 : node.height) ?? 0) / 1.5 - labelPaddingY * 2,
      10
    )
  }
  const { shapeSvg, bbox, label } = await labelHelper(parent, node, getNodeClasses(node))
...
function stateEnd(parent, node, { config: { themeVariables } }) {
  ...
  if (node.width < 25 && nodeShadow && node.look !== 'handDrawn') {
    const svgId =
      ((_b = (_a = parent.node()) == null ? void 0 : _a.ownerSVGElement) == null
        ? void 0
        : _b.id) ?? ''
...
async function subroutine(parent, node) {
  ...
  const { shapeSvg, bbox } = await labelHelper(parent, node, getNodeClasses(node))
  const totalWidth =
    ((node == null ? void 0 : node.width) ?? bbox.width) + 2 * FRAME_WIDTH2 + labelPaddingX
...
async function taggedRect(parent, node) {
  ...
  if (node.width || node.height) {
    ...
    node.width = Math.max(
      ((node == null ? void 0 : node.width) ?? 0) -
        labelPaddingX * 2 -
        TAG_RATIO * (node.height + labelPaddingY * 2),
      10
    )
  }
  const { shapeSvg, bbox } = await labelHelper(parent, node, getNodeClasses(node))
...
async function text(parent, node) {
  ...
  const { shapeSvg, bbox } = await labelHelper(parent, node, getNodeClasses(node))
  const totalWidth = Math.max(
    bbox.width + (node.padding ?? 0),
    (node == null ? void 0 : node.width) || 0
  )
  const totalHeight = Math.max(
    bbox.height + (node.padding ?? 0),
    (node == null ? void 0 : node.height) || 0
  )
  const x = -totalWidth / 2
  const y = -totalHeight / 2
...
async function windowPane(parent, node) {
  ...
  if (node.width || node.height) {
    node.width = Math.max(
      ((node == null ? void 0 : node.width) ?? 0) - paddingX * 2 - rectOffset,
      10
    )
    node.height = Math.max(
      ((node == null ? void 0 : node.height) ?? 0) - paddingY * 2 - rectOffset,
      10
    )
  }
  const { shapeSvg, bbox, label } = await labelHelper(parent, node, getNodeClasses(node))
...
async function erBox(parent, node) {
  ...
  if (entityNode.attributes.length === 0 && node.label) {
    ...
    if (
      calculateTextWidth(node.label, config) + options2.labelPaddingX * 2 <
  ...
  for (const attribute of entityNode.attributes) {
    ...
    const rowHeight =
      Math.max(typeBBox.height, nameBBox2.height, keysBBox.height, commentBBox.height) +
      TEXT_PADDING
  ...
  if (rows.length > 0) {
    totalShapeBBoxHeight = rows.reduce(
      (sum, row) => sum + ((row == null ? void 0 : row.rowHeight) ?? 0),
      0
    )
  }
  const w = Math.max(
    shapeBBox.width + PADDING * 2,
    (node == null ? void 0 : node.width) || 0,
    maxWidth
  )
  const h = Math.max(
    (totalShapeBBoxHeight ?? 0) + nameBBox.height,
    (node == null ? void 0 : node.height) || 0
  )
  const x = -w / 2
  const y = -h / 2
  ...
  for (const [i, row] of rows.entries()) {
    ...
    const roughRect2 = rc.rectangle(
      x,
      nameBBox.height + y + (row == null ? void 0 : row.yOffset),
      w,
      row == null ? void 0 : row.rowHeight,
      {
        ...options,
          ...
          (img) =>
            new Promise((res) => {
              function setupImage() {
                ...
                if (noImgText) {
                  const bodyFontSize =
                    ((_a = config.fontSize) == null ? void 0 : _a.toString()) ??
...
async function classBox(parent, node) {
  ...
  const renderExtraBox =
    ...
    !((_b = config.class) == null ? void 0 : _b.hideEmptyMembersBox)
  const rc = at.svg(shapeSvg)
  const options = userNodeOverrides(node, {})
  if (node.look !== 'handDrawn') {
    options.roughness = 0
  ...
  shapeSvg.selectAll('.text').each((_, i, nodes) => {
    ...
    if (
      classNode.members.length === 0 &&
      classNode.methods.length === 0 &&
      ((_a2 = config.class) == null ? void 0 : _a2.hideEmptyMembersBox)
    ) {
    ...
    let newTranslateX = x
    if (
      text2.attr('class').includes('label-group') ||
      text2.attr('class').includes('annotation-group')
    ) {
...
async function requirementBox(parent, node) {
  ...
  const totalWidth =
    (((_a = shapeSvg.node()) == null ? void 0 : _a.getBBox().width) ?? 200) + padding
  const totalHeight =
    (((_b = shapeSvg.node()) == null ? void 0 : _b.getBBox().height) ?? 200) + padding
  const x = -totalWidth / 2
  const y = -totalHeight / 2
  const rc = at.svg(shapeSvg)
  const options = userNodeOverrides(node, {})
  ...
  if (
    ...
    (requirementEdgeLabelBackground ||
      (borderColorArray == null ? void 0 : borderColorArray.length))
  ) {
    shapeSvg.selectAll('path').attr('style', nodeStyles)
  }
  return shapeSvg
...
async function kanbanItem(parent, kanbanNode, { config }) {
  ...
  let link
  if (
    'ticket' in kanbanNode &&
    kanbanNode.ticket &&
    ((_a = config == null ? void 0 : config.kanban) == null ? void 0 : _a.ticketBaseUrl)
  ) {
    ticketUrl =
      (_b = config == null ? void 0 : config.kanban) == null
        ? void 0
        : _b.ticketBaseUrl.replace('#TICKET#', kanbanNode.ticket)
    link = shapeSvg
  ...
  const totalHeight =
    Math.max(
      bbox.height + labelPaddingY * 2,
      (kanbanNode == null ? void 0 : kanbanNode.height) || 0
    ) + heightAdj
  const x = -totalWidth / 2
  const y = -totalHeight / 2
  labelElTitle.attr(
...
//# sourceMappingURL=chunk-NCFOLWPN.js.map
