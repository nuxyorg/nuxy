...
import { isEmResetFrame } from './chunk-TZ7GS4SL.js'
import { calculateTextDimensions, cleanAndMerge, wrapLabel } from './chunk-N5H3GFM2.js'
import {
  clear,
...
function evolveFramePositioned(state, _event) {
  ...
  const previousSwimlane =
    state.previousSwimlaneNumber !== void 0 ? state.swimlanes[state.previousSwimlaneNumber] : void 0
  const dimension = {
    width:
      Math.max(
...
function hasSourceFrame(frame) {
  return (
    frame.sourceFrames !== void 0 && frame.sourceFrames !== null && frame.sourceFrames.length > 0
  )
}
...
function renderD3Box(diagram2, diagramProps2) {
  return (box) => {
    ...
    const f = g
      ...
      .attr('x', box.x + diagramProps2.boxPadding)
      .attr('y', y + 10)
      .attr('width', box.dimension.width - 2 * diagramProps2.boxPadding)
      .attr('height', box.dimension.height - 2 * diagramProps2.boxPadding)
    const text = f
      .append('xhtml:div')
      .style('display', 'table')
      .style('height', '100%')
...
function renderD3Swimlane(diagram2, maxR, diagramProps2, themeVariables) {
  return (swimlane) => {
    ...
    g.append('rect')
      ...
      .attr('y', swimlane.y)
      .attr('rx', '3')
      .attr('width', maxR + diagramProps2.swimlanePadding)
      .attr('height', swimlane.height)
      .attr('fill', oddBackground)
      .attr('stroke', backgroundStroke)
    g.append('text')
      .attr('font-weight', diagramProps2.swimlaneTextFontWeight)
      .attr('x', 30)
...
var draw = __name(function (txt, id, ver, diagObj) {
  ...
  setupGraphViewbox2(
    void 0,
    diagram2,
    (config == null ? void 0 : config.padding) ?? 30,
    config == null ? void 0 : config.useMaxWidth
  )
}, 'draw')
...
export { diagram }
//# sourceMappingURL=diagram-KO2AKTUF-QSZSQFM3.js.map
