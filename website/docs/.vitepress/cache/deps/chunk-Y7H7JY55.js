...
function longestPath(g) {
  ...
  function dfs3(v) {
    ...
    if (
      rank2 === Number.POSITIVE_INFINITY || // return value of _.map([]) for Lodash 3
...
function enterEdge(t, g, edge) {
  ...
  var candidates = filter_default(g.edges(), function (edge2) {
    return (
      flip === isDescendant(t, t.node(edge2.v), tailLabel) &&
      flip !== isDescendant(t, t.node(edge2.w), tailLabel)
    )
  })
...
function buildLayerGraph(g, rank2, relationship) {
  var root = createRootNode(g),
    result = new Graph({ compound: true }).setGraph({ root }).setDefaultNodeLabel(function (v) {
      return g.node(v)
    })
  forEach_default(g.nodes(), function (v) {
    var node = g.node(v),
    ...
    if (node.rank === rank2 || (node.minRank <= rank2 && rank2 <= node.maxRank)) {
      ...
      forEach_default(g[relationship](v), function (e) {
        var u = e.v === v ? e.w : e.v,
          ...
          weight = !isUndefined_default(edge) ? edge.weight : 0
...
function doResolveConflicts(sourceSet) {
  ...
  function handleIn(vEntry) {
    return function (uEntry) {
      ...
      if (
        isUndefined_default(uEntry.barycenter) ||
        isUndefined_default(vEntry.barycenter) ||
...
function sort(entries, biasRight) {
  ...
  var sortable = parts.lhs,
    unsortable = sortBy_default(parts.rhs, function (entry) {
      return -entry.i
    }),
    vs = [],
    sum = 0,
...
function order(g) {
  var maxRank2 = maxRank(g),
    downLayerGraphs = buildLayerGraphs(g, range_default(1, maxRank2 + 1), 'inEdges'),
    upLayerGraphs = buildLayerGraphs(g, range_default(maxRank2 - 1, -1, -1), 'outEdges')
...
function alignCoordinates(xss, alignTo) {
  var alignToVals = values_default(alignTo),
    alignToMin = min_default(alignToVals),
    alignToMax = max_default(alignToVals)
...
export { layout }
//# sourceMappingURL=chunk-Y7H7JY55.js.map
