---
title: Extension Access & Permissions
---

<div v-pre>

# Extension Access & Permissions
...
| **`clipboard` (consent)** | Same `core.clipboard`                    | User prompt before read/write (see [Security](/design/security))    |
...
Exposed on `window.core` via `contextBridge` (`preload.ts`). Used by extension UIs loaded from `nuxy-ext://<manifest.id>/…` and by the main shell.
...
| **Shared UI kit**       | `window.UI`                       | Set by `ui-default` extension from `@nuxy/ui`                   |
| **Bootstrap shell**     | `com.nuxy.shell` extension        | Renderer loads `nuxy-ext://com.nuxy.shell/frontend.js`          |
...
| **Typed `IpcResponse`** | Wrapper on all invokes           | Documented in [API Design](/design/api-design); worker returns raw `{ result, error }` today |
...
| `com.nuxy.calculator` | `provider` | `eval` (shell always uses this name for providers) | `logger` (optional); evaluates math in worker |
...
| Extension  | Storage | DB  | Network | Media | Custom IPC         |
| ---------- | ------- | --- | ------- | ----- | ------------------ |
| Notes      | Yes     | Yes | No      | No    | CRUD note channels |
| Calculator | No      | No  | No      | No    | `eval` only        |
| Settings   | Yes     | No  | No      | No    | config channels    |
...
See [Security & Strict Isolation](/design/security) for the full threat model and consent flow.
...
| ------------------------------------- | -------------------------------------------------- | --------------------------------------- |
| Module layout and manifest example    | [Modules](/design/modules)                         | Extension folder structure              |
| CoreContext and IPC contracts         | [API Design](/design/api-design)                   | IpcResponse wrapper, validation pattern |
| Isolation, chroot, permission prompts | [Security](/design/security)                       | Full threat model and consent flow      |
| Tool / Provider / Orchestrator roles  | [Omni Input System](/design/omni-input-system)     | Capability flags and routing            |
| UI loading and omni bar events        | [Frontend Extensions](/design/frontend-extensions) | Canvas zones and shortcut management    |
| Plugin system and CoreContext proxy   | [Plugin System](/design/modular-plugin-system)     | Thread isolation and MessagePort detail |
| API reference (curated)               | [API Overview](/api/)                              | CoreContext, IPC, storage, window APIs  |

---

**See also:** [Development Guide](/extensions/development-guide) · [Security](/design/security) · [Home](/)
