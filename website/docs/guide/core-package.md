...
#### `extToType(ext: string): QueryType | null` *(Internal)*
Maps specific file extensions (images, videos, audio, archives, PDFs) to their corresponding generic query types.

#### `classifyFromExt(ext: string, ctx: QueryContext, types: QueryType[]): void` *(Internal)*
