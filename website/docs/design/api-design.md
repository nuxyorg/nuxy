...
Electron's `contextBridge` is the only safe way to pass data between the Chromium renderer and the Node.js backend. To prevent "wildcard" vulnerabilities, Nuxy strictly forbids exposing raw `ipcRenderer.send` or `.invoke`.
...
To ensure extension frontends can access `window.core.ipc` with TypeScript autocomplete, we declare the global interface in the root `env.d.ts`.
...
**See also:** [Modules](/design/modules) · [Extension Access](/extensions/extension-access) · [API Reference](/api/)
...
| ----------------------------------- | ------------------------------------------------ | --------------------------------------------------------------- |
| Extension access and permissions    | [Extension Access](/extensions/extension-access) | Full API implementation status, renderer bridge, manifest rules |
| Plugin system and CoreContext proxy | [Plugin System](/design/modular-plugin-system)   | How CoreContext is injected via MessagePort                     |
| Security model and IPC trust        | [Security](/design/security)                     | Why the backend must never trust IPC payloads                   |
