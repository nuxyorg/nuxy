...
/**
 * Load and cache translations for `extId` from the kernel.
 * Re-fetches automatically when the `nuxy-locale-changed` event fires
 * (emitted by the settings extension when the user changes language preferences).
 *
 * Usage:
 * ```tsx
 * const EXT_ID = 'com.nuxy.my-extension'
 * export default function MyView() {
 *   const { t } = useTranslation(EXT_ID)
