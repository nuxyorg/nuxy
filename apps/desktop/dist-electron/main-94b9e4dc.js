...
import { BrowserWindow, screen, protocol, net, clipboard, ipcMain, app } from "electron";
...
async function scanExtensions() {
  ...
  {
    try {
      const { copyDefaultExtensions } = await import("./extensions-b21c8724.js");
...
function registerIpc() {
  ...
  ipcMain.on("window:resize", (event, width, height) => {
    ...
    if ((_a = global.isHoppidikActive) == null ? void 0 : _a.call(global)) {
  ...
  ipcMain.on("window:hide", (event) => {
    ...
    if (win) {
      {
        (_a = global.clearHoppidik) == null ? void 0 : _a.call(global);
      }
      win.hide();
    } else {
  ...
  {
    import("./hoppidik-3fab8978.js").then(({ registerHoppidikHandlers }) => {
      registerHoppidikHandlers();
    }).catch((err) => {
      log$1.error("Failed to register hoppidik handlers:", err);
    });
  }
