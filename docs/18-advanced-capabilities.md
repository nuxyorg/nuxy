...
- **Resolution**: If Extension A requires Extension B (e.g., `"com.nuxy.vault": "^1.2.0"`), but Extension B is missing, Extension A is placed in a `Suspended` state. The React UI will prompt the user: _"AI Orchestrator requires the Vault extension to function. Click here to download it."_
