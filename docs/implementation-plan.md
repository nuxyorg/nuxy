...
**Steps:**
1. **`apps/desktop/package.json`**: Initialize Vite + React + Electron dependencies.
2. **`apps/desktop/vite.config.ts`**: Configure Vite to build for Electron renderer and main process.
3. **`apps/desktop/electron/main.ts`**: 
   - Boot Electron app.
   - Request single instance lock (`app.requestSingleInstanceLock()`).
4. **`apps/desktop/electron/window.ts`**:
...
5. **`apps/desktop/src/main.tsx` & `App.tsx`**:
   - Render a centered, styled "OmniBar" `div` using tailwind.
   - Display a fallback message: *"Nuxy MVP: Ready"*.
