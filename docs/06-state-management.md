...
Because extensions are isolated, they *cannot* share React Context. If the UI Theme changes, Nuxy Core passes it down via CSS variables (e.g. `--background`). The extension UI doesn't need a React Context to know it's in dark mode; it just uses `@nuxy/ui` components which automatically adapt to the CSS variables.
