...
const { List, ListItem, ListItemBody, ListItemText, ItemLeading, useToolKeyActions, EmptyState } =
  window.UI
...
export default function MyTool({ query, extensionId }) {
  ...
  useToolKeyActions([
    {
      ...
      label: 'Previous',
      hint: '↑↓',
      handler: () => setSelected((i) => Math.max(0, i - 1)),
    },
    {
      key: 'ArrowDown',
      label: 'Next',
      handler: () => setSelected((i) => Math.min(items.length - 1, i + 1)),
    },
  ...
  useEffect(() => {
    fetch(`nuxy-ext://${extensionId}/data.json`)
      .then((r) => r.json())
      .then(setItems)
  }, [extensionId])
