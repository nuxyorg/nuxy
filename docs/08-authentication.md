...
If the AI Orchestrator extension needs an API key (e.g., Gemini) to function, it _does not_ ask the user directly. It asks the Vault via the Nuxy Message Broker.
...
- **No Disk Storage of Passwords**: The Master Password or the derived AES key is _never_ written to disk by the Vault extension.
