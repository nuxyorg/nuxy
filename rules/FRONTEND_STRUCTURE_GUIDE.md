...
> Companion to `EXTENSION_GUIDE.md` §5. Prescribes _when_ and _how_ to split the custom element and its controller. The notes extension (`extensions/notes/`) is the reference implementation.
...
### 1. Vanilla HTMLElement + h() helper (DEPRECATED - Legacy)
...
export class NuxyToolMyExtensionElement extends HTMLElement implements NuxyToolElement {
  ...
  set query(v: string) { this._query = v; this.controller?.setQuery(v) }
  get query() { return this._query }
  set committedQuery(v: string) { this._committedQuery = v }
  get committedQuery() { return this._committedQuery }
  set extensionId(v: string) { this._extensionId = v }
...
customElements.define('nuxy-tool-my-extension', NuxyToolMyExtensionElement)
```

---

### 2. LitElement (preferred for complex UIs)
...
export class NuxyToolMyExtensionElement extends LitElement implements NuxyToolElement {
  ...
  render() {
    ...
    return html`
      ${this.items.length === 0
        ? EmptyState?.({ message: 'No results' })
        : List?.({ children: this.items.map(item => /* ... */) })
      }
    `
  }
}
...
## Reference: notes extension after refactor
