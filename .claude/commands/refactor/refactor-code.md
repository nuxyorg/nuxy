...
- Update any ARCHITECTURE.md, DESIGN.md, or similar files only if they exist
...
- Look for project-specific documentation files (CLAUDE.md, CONTRIBUTING.md, etc.)
...
When invoked with target file(s), this command will:
...
Use the Write tool to create the report file at:
```
...
Analyze the following file(s) for refactoring opportunities:
...
**FINAL REMINDER**: This is ANALYSIS ONLY — do not modify any code. Your ONLY output should be the report file in the `reports/refactor/` directory.
