...
const app = await electron.launch({
  ...
  args: ['--no-sandbox', `--user-data-dir=${userDataDir}`,
    '--disable-renderer-backgrounding', '--disable-background-timer-throttling', APP_DIR],
  env: { ...cleanEnv, DISPLAY: process.env.DISPLAY ?? ':0',
    ELECTRON_OZONE_PLATFORM_HINT: 'x11', NUXY_DATA_DIR: nuxyDataDir,
...
if (await extTab.count() > 0) {
  ...
  const panelText = await page.locator('[class*="scroll"], [class*="right"], main, [class*="panel"]')
    .first().allTextContents().catch(() => ['(none)'])
