...
export function registerIpc() {
  ...
  ipcMain.handle(
    'ext:invoke',
    async (event, extId: string, channel: string, payload: unknown) => {
      ...
      if (extId === 'kernel' || extId === 'core') {
        if (channel === 'listTools') {
          const tools = loadedExtensions.filter(
            (ext: any) => ext.manifest && ext.manifest.type === 'tool'
          )
        ...
        if (channel === 'listProviders') {
          const providers = loadedExtensions.filter(
            (ext: any) => ext.manifest && ext.manifest.type === 'provider'
          )
      ...
      return new Promise((resolve) => {
        ...
        log.silly(`Dispatching message to worker "${extId}"`, {
          msgId,
          channel,
          payload
        })
        ...
        const listener = (msg: any) => {
          if (msg.id === msgId) {
            ...
            if (msg.error) {
              log.warn(
                `Worker "${extId}" replied with error on channel "${channel}"`,
                msg.error
              )
              resolve({ success: false, error: msg.error })
            } else {
              log.silly(
                `Worker "${extId}" replied on channel "${channel}"`,
                msg.result
              )
  ...
  ipcMain.on('window:resize', (event, width: number, height: number) => {
    ...
    if (!win) {
      log.warn('window:resize — no BrowserWindow for sender')
      return
    }
    ...
    try {
      ...
      if (!win.isVisible() || ch < MIN_CONTENT_HEIGHT) {
        spring.snapToTarget()
        log.info(
          `Window snapped instantly to target size (${targetW}x${targetH})`
        )
      }
    } catch (error) {
  ...
  ipcMain.on('window:dragEnd', (event) => {
    ...
    log.silly(
      `window:dragEnd — spring synced to ${JSON.stringify(win.getBounds())}`
    )
  })
  ...
  if (import.meta.env.DEV) {
    import('./dev/hoppidik.js')
      .then(({ registerHoppidikHandlers }) => {
  ...
  log.info(
    `IPC handlers registered: [ext:invoke, window:resize, window:center, window:hide${import.meta.env.DEV ? ', window:startHoppidik, window:stopHoppidik' : ''}]`
  )
}
